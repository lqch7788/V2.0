 ...
 * 获取招聘申请列表
 */
export async function getRecruitmentRecords(filters, pagination) {
  const params = {};
...
export async function getRecruitmentById(id) {
  const response = await enhancedApiClient.get(`/recruitment/${id}`);
 ...
 * 创建招聘申请
 */
export async function createRecruitmentRecord(recruitment) {
  const snakeData = {
  ...
  const response = await enhancedApiClient.post('/recruitment', snakeData);
 ...
 * 更新招聘申请
 */
export async function updateRecruitmentRecord(id, updates) {
  const snakeData = {};
 ...
 * 删除招聘申请
 */
export async function deleteRecruitmentRecord(id) {
