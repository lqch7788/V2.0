 ...
 *
 * 使用场景: apiClient.js 等非React/Vue模块
...
/**
 * 从Store惰性读取字符串配置值
 * @param {string} key - 命名空间配置键
 * @param {string} defaultValue - 配置缺失时的兜底值
 * @returns {string}
 */
export function getSystemConfigValue(key, defaultValue) {
  // 先查缓存
 ...
 * @returns {number}
 */
export function getSystemConfigValueNumber(key, defaultValue) {
 ...
 * @returns {boolean}
 */
export function getSystemConfigValueBoolean(key, defaultValue) {
  const strVal = getSystemConfigValue(key, String(defaultValue));
 ...
 */
export function getSystemConfigValuesByPrefix(prefix) {
  try {
    const { useSystemConfigStore } = require('../stores');
    const store = useSystemConfigStore();
    const configs = store.configs || [];
