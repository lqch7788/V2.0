<template>
  <div v-if="recommendations.length === 0" class="flex flex-col items-center justify-center py-12 text-gray-500">
  ...
  <div v-else class="space-y-3">
    <div
      ...
      :key="worker.workerId"
      :class="index === 0 ? 'cursor-pointer' : 'cursor-pointer'"
    >
      <DispatchRecommendCard
...
const DispatchRecommendCard = {
  props: ['recommendation', 'rank', 'isTopPick'],
  template: `
    <div
      :class="[
    ...
    >
      ...
      <!-- 推荐理由 -->
      <div v-if="recommendation.reasons.length > 0" class="flex flex-wrap gap-1">
        <span
