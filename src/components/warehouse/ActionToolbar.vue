  ...
  <div class="flex items-center justify-between bg-white rounded-xl p-4 shadow-sm">
    ...
    <div class="flex gap-2">
      ...
      <template v-if="!batchEditMode && !deleteMode && !exportMode">
        <el-button v-if="canCreate && onAdd" type="primary" size="small" @click="onAdd">
          <el-icon><Plus /></el-icon>
          新增
        </el-button>
        <el-button v-if="showLowStockButton" :type="filters.showLowStock ? 'danger' : 'default'" size="small" @click="onLowStockToggle">
          <span v-if="lowStockCount > 0" class="bg-red-500 text-white text-xs px-1.5 py-0.5 rounded-full mr-1">{{ lowStockCount }}</span>
          库存不足
        </el-button>
        <el-button v-if="canEdit" type="primary" size="small" @click="onBatchEdit">编辑</el-button>
        <el-button v-if="canDelete" type="danger" size="small" @click="onDelete">删除</el-button>
...
const props = defineProps({
  ...
  showLowStockButton: { type: Boolean, default: true }
})

const emit = defineEmits([
  'low-stock-toggle', 'batch-edit', 'delete', 'export',
  'confirm-batch-edit', 'cancel-batch-edit',
  'confirm-delete', 'cancel-delete',
  'confirm-export', 'cancel-export', 'add'
])
