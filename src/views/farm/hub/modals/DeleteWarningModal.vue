<!--
  删除警告弹窗（通用）
  对标 V1.1 src/components/farm/hub/modals/DeleteWarningModal.tsx
-->
<template>
  <el-dialog v-model="visible" title="删除警告" width="480px" v-dialog-draggable @close="$emit('update:modelValue', false)">
    <div class="flex items-start gap-3">
      ...
      <div>
        <p class="font-semibold mb-2">确定要删除 {{ records?.length || 1 }} 条记录吗？</p>
        <p class="text-sm text-red-600">⚠️ 删除后无法恢复，请谨慎操作</p>
      </div>
    </div>
    <template #footer>
