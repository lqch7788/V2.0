<template>
  <div class="card-greenhouse animate-card-in" style="animation-delay: 0.2s">
    ...
    <div v-if="expanded" class="max-h-60 overflow-y-auto scrollbar-natural">
      <table class="w-full text-sm">
        <thead class="table-header text-white sticky top-0">
          <tr>
            <th class="px-3 py-2 text-left font-semibold">棚号</th>
            <th class="px-3 py-2 text-left font-semibold">作物</th>
            <th class="px-3 py-2 text-left font-semibold">面积(㎡)</th>
            <th class="px-3 py-2 text-left font-semibold">温室类型</th>
        ...
        <tbody class="divide-y divide-green-200">
          <tr v-for="item in greenhouseData" :key="item.no">
            ...
            <td class="px-3 py-2 text-center">
              <el-button size="small" text class="btn-detail" @click="$emit('detail-click', { type: 'greenhouse', data: item })">
                详情&gt;
              </el-button>
            </td>
          </tr>
...
const getStatusClass = (status) => {
  switch (status) {
    ...
    case '育苗中': return 'status-seedling'
    case '待种植': return 'status-waiting'
