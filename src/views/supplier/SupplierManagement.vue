<template>
  <div class="space-y-6">
    ...
    <SupplierDetailModal :is-open="showDetailModal" @close="showDetailModal = false" />

    <!-- 新增/编辑弹窗 - ElModal（内联实现），与V1.1 SupplierAddModal/SupplierEditModal一致 -->
    <ElModal
      :model-value="showFormModal"
      :title="isEdit ? '编辑供应商' : '新增供应商'"
      :width="1152"
      :height="600"
      @update:model-value="(v) => { if (!v) { showFormModal = false; resetForm() } }"
      @close="showFormModal = false; resetForm()"
    >
      <!-- 基本信息区域 - 浅绿背景 -->
      <div class="p-4 bg-emerald-50 border-b border-gray-200 -mx-4 -mt-4">
        <div class="grid grid-cols-4 gap-3">
          <div>
            ...
            <input v-model="form.code" :disabled="isEdit" placeholder="手动输入或使用编码生成器" class="w-full h-9 px-3 border border-gray-500 rounded text-sm bg-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 disabled:bg-gray-100 disabled:cursor-not-allowed" />
          </div>
          <div>
            <label class="block text-xs font-medium text-emerald-700 mb-1">供应商名称 *</label>
            <input v-model="form.name" class="w-full h-9 px-3 border border-gray-500 rounded text-sm bg-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
          </div>
          <div>
            <label class="block text-xs font-medium text-emerald-700 mb-1">供应类型 *</label>
            <select v-model="form.supplierType" class="w-full h-9 px-3 border border-gray-500 rounded text-sm bg-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200">
          ...
          <div>
            ...
            <select v-model="form.supplierAttribute" class="w-full h-9 px-3 border border-gray-500 rounded text-sm bg-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200">
          ...
          <div>
            ...
            <select v-model="form.organization" class="w-full h-9 px-3 border border-gray-500 rounded text-sm bg-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200">
          ...
          <div>
            ...
            <input v-model="form.contact" class="w-full h-9 px-3 border border-gray-500 rounded text-sm bg-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
          </div>
          <div>
            <label class="block text-xs font-medium text-emerald-700 mb-1">移动电话 *</label>
            <input v-model="form.mobilePhone" class="w-full h-9 px-3 border border-gray-500 rounded text-sm bg-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
          </div>
          <div>
            <label class="block text-xs font-medium text-emerald-700 mb-1">状态</label>
            <select v-model="form.status" class="w-full h-9 px-3 border border-gray-500 rounded text-sm bg-white focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200">
              <option value="合作中">合作中</option>
              <option value="暂停">暂停</option>
              <option value="终止">终止</option>
            </select>
      ...
      <div class="flex-1 overflow-y-auto p-4">
        <div class="grid grid-cols-3 gap-3">
          <div>
            ...
            <input v-model="form.workPhone" class="w-full h-9 px-3 border border-gray-500 rounded text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
          </div>
          <div>
            <label class="block text-xs font-medium text-gray-700 mb-1">传真</label>
            <input v-model="form.fax" class="w-full h-9 px-3 border border-gray-500 rounded text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
          </div>
          <div>
            <label class="block text-xs font-medium text-gray-700 mb-1">国家</label>
            <input v-model="form.country" class="w-full h-9 px-3 border border-gray-500 rounded text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
          </div>
          ...
          <div>
            ...
            <input type="date" v-model="form.createDate" class="w-full h-9 px-3 border border-gray-500 rounded text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
          </div>
          <div class="col-span-3">
            <label class="block text-xs font-medium text-gray-700 mb-1">详细地址</label>
            <input v-model="form.address" class="w-full h-9 px-3 border border-gray-500 rounded text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
          </div>
          <div>
            <label class="block text-xs font-medium text-gray-700 mb-1">开户行</label>
            <input v-model="form.bankName" class="w-full h-9 px-3 border border-gray-500 rounded text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
          </div>
          <div class="col-span-2">
            <label class="block text-xs font-medium text-gray-700 mb-1">银行卡号</label>
            <input v-model="form.bankCardNumber" class="w-full h-9 px-3 border border-gray-500 rounded text-sm font-mono focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200" />
          </div>
          <div class="col-span-3">
            <label class="block text-xs font-medium text-gray-700 mb-1">备注</label>
            <textarea v-model="form.remarks" rows="2" class="w-full px-3 py-2 border border-gray-500 rounded text-sm focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200"></textarea>
          </div>
        </div>
      </div>
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          <el-button size="small" @click="showFormModal = false; resetForm()">
            <X class="w-4 h-4 mr-1" />取消
          </el-button>
          <el-button type="primary" size="small" @click="handleSave">
            <Send class="w-4 h-4 mr-1" />{{ isEdit ? '保存' : '提交' }}
          </el-button>
        </div>
      </template>
    </ElModal>

    <!-- 批量编辑弹窗逻辑保留在父组件（ElModal 内联实现），与V1.1 SupplierBatchEditModal一致：逐条编辑+累积保存 -->
