        ...
        <div class="relative w-40 h-40 mx-auto mt-4">
          ...
          <div class="absolute inset-0 flex flex-col items-center justify-center">
            <span :class="['text-lg font-bold', scoreColor(overallScore)]">
              {{ overallScore >= 80 ? '优秀' : overallScore >= 60 ? '良好' : '待改进' }}
            </span>
          </div>
        </div>
      </div>

      <!-- 仪表盘行：产量达成率 + 任务完成 + 成本控制 -->
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col items-center">
          ...
          <div class="mt-4 text-center">
            <p class="text-sm text-gray-500">
              ...
              {' / '}
              总计 <span class="font-medium text-gray-700">{{ indicator?.task.total ?? 0 }}</span> 项            </p>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col items-center">
          ...
          <div class="mt-4 text-center">
            ...
            <p class="text-xs text-gray-400 mt-1">
              工人：{{ indicator?.labor.workerCount ?? 0 }}人 | 工时：{{ indicator?.labor.totalHours ?? 0 }}h
            </p>
          </div>
        </div>
      </div>
...
const TrafficLight = {
  ...
  setup(props) {
    ...
    // 修复：使用s === props.status 而不width === status
...
const costControlRate = computed(() => {
  ...
  // 与V1.1保持一致  const rate = Math.round((1 - (indicator.value.labor.totalCost / budget)) * 100 + 100)
...
const getDateRangeByPeriod = (mode) => {
  ...
  switch (mode) {
    case 'month':
      // 本月第一天      startDate = new Date(year, month, 1)
      endDate = today
      break
    case 'quarter':
      // 本季度第一天      startDate = new Date(year, quarter * 3, 1)
      endDate = today
      break
    case 'year':
    default:
      // 本年度第一天      startDate = new Date(year, 0, 1)
      endDate = today
      break
  }
