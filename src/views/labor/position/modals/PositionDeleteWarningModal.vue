<!--
  岗位删除警告弹窗
  对标 V1.1 src/components/labor/position/modals/PositionDeleteWarningModal.tsx
-->
<template>
  <el-dialog v-model="visible" title="删除警告" width="480px" v-dialog-draggable @close="$emit('update:modelValue', false)">
    <div class="flex items-start gap-3">
      ...
      <div>
        <p class="font-semibold mb-2">确定删除 {{ records?.length || 1 }} 个岗位？</p>
        <p class="text-sm text-red-600">⚠️ 删除后无法恢复，关联员工将受影响</p>
      </div>
    </div>
    <template #footer>
