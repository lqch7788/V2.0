<!--
  请假删除警告弹窗
  对标 V1.1 src/components/labor/leave/modals/LeaveDeleteWarningModal.tsx
-->
<template>
  <el-dialog v-model="visible" title="删除警告" width="480px" v-dialog-draggable @close="$emit('update:modelValue', false)">
    <div class="flex items-start gap-3">
      ...
      <div>
        <p class="font-semibold mb-2">确定删除 {{ records?.length || 0 }} 条请假申请？</p>
        <p class="text-sm text-red-600">⚠️ 删除后无法恢复，请确认操作</p>
      </div>
    </div>
    <template #footer>
