    ...
    <div class="bg-white rounded-xl p-6 shadow-sm">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-gray-500">指标发布/调整、预算编制/调整审批</p>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
      ...
      <el-dialog v-model="detailVisible" title="审批详情" width="800px" destroy-on-close>
        <div v-if="currentApproval" class="space-y-4">
          ...
          <!-- 业务关联信息 -->
          <div v-if="currentApproval.businessLink" class="border-t pt-4">
            <h4 class="text-sm font-semibold text-gray-900 mb-3">关联业务数据</h4>
            <pre class="text-xs text-gray-600 bg-gray-50 rounded p-3 overflow-auto max-h-48">{{ JSON.stringify(currentApproval.businessLink, null, 2) }}</pre>
          </div>
        </div>
      </el-dialog>
    </div>
...
// Tab配置
const tabs = [
  { key: 'indicator', label: '指标发布', icon: Aim, types: [ApprovalType.INDICATOR_APPROVAL] },
  { key: 'indicator_adjust', label: '指标调整', icon: TrendCharts, types: [ApprovalType.INDICATOR_ADJUST] },
  { key: 'budget_create', label: '预算编制', icon: Coin, types: [ApprovalType.BUDGET_CREATE] },
  { key: 'budget_adjust', label: '预算调整', icon: TrendCharts, types: [ApprovalType.BUDGET_ADJUST] }
]
