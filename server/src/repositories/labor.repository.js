...
export class LaborRepository {
    ...
    async employeeLeave(id, leaveDate, leaveReason) {
        const db = getDatabase();
        const now = new Date().toISOString();
        db.run(`UPDATE employees SET status = 'left', leave_date = ?, leave_reason = ?, update_time = ? WHERE id = ?`, [leaveDate, leaveReason, now, id]);
        saveDatabase();
    }
    /**
     * 工人复职
    ...
    async employeeRejoin(id, rejoinDate) {
        const db = getDatabase();
        const now = new Date().toISOString();
        db.run(`UPDATE employees SET status = 'active', leave_date = NULL, leave_reason = NULL, update_time = ? WHERE id = ?`, [now, id]);
        saveDatabase();
    }
    /**
     * 获取员工统计
