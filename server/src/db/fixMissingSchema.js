...
export async function fixMissingSchema(){
  ...
  try {
    ...
    const nullCount = db.exec(`SELECT COUNT(*)`);
...
export function deduplicateDictionaries(){
  ...
  const stmt = db.prepare(`
    SELECT category_code, dict_code, COUNT(*)= 'active'
    GROUP BY category_code, dict_code
    HAVING cnt > 1
  `);

  const duplicates: Array<{ category_code: string; dict_code: string; cnt= [];
  while (stmt.step()) {
    const row = stmt.getAsObject(): string; dict_code: string; cnt: number };
    duplicates.push(row);
  }
  stmt.free();
