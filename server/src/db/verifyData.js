...
function getRecordCount(tableName){
  const db = getDatabase();
  const result = db.exec(`SELECT COUNT(*)${tableName}`);
...
function getTableColumns(tableName){
  ...
  return queryToArray<any>(result).map((col) => col.name);
}
...
){ requiredFields = [], uniqueFields = [], foreignKeys = [] } = options;
  const problems= [];
 ...
 * 生成报告
 */
export function generateReport(report?: VerificationReport){
 ...
 * 打印报告到控制台
 */
export function printReport(report?: VerificationReport){
