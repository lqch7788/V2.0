...
function getTableColumns(tableName) {
    ...
    return queryToArray(result).map((col) => col.name);
}
/**
 * 添加问题
 */
function addProblem(problems, level, table, message, code, field, recordId) {
    problems.push({
        table,
        field,
        recordId,
 ...
 * 验证必填字段
 */
function verifyRequiredFields(tableName, requiredFields) {
    const problems = [];
    ...
    records.forEach((record) => {
        requiredFields.forEach((field) => {
            ...
            if (value === null || value === undefined || value === '') {
                addProblem(problems, 'error', tableName, `必填字段 "${field}" 为空或缺失`, 'REQUIRED_FIELD_EMPTY', field, record.id || record.oid || record.code);
            }
        });
    });
    return problems;
 ...
 * 验证外键关联
 */
function verifyForeignKey(tableName, foreignKeyField, referencedTable, referencedField = 'id') {
    const problems = [];
    const records = getAllRecords(tableName);
    const referencedRecords = getAllRecords(referencedTable);
    const validIds = new Set(referencedRecords.map((r) => r[referencedField]));
    records.forEach((record) => {
        const fkValue = record[foreignKeyField];
        if (fkValue && !validIds.has(fkValue)) {
            addProblem(problems, 'error', tableName, `外键 "${foreignKeyField}" 的值 "${fkValue}" 在表 "${referencedTable}" 中不存在`, 'FOREIGN_KEY_INVALID', foreignKeyField, record.id || record.oid || record.code);
        }
    });
    return problems;
}
 ...
 * 验证唯一性字段
 */
function verifyUniqueFields(tableName, uniqueFields) {
    const problems = [];
    const records = getAllRecords(tableName);
    const seen = new Map();
    // 初始化跟踪Map
    ...
    records.forEach((record) => {
        uniqueFields.forEach((field) => {
            ...
            if (existing) {
                addProblem(problems, 'error', tableName, `字段 "${field}" 的值 "${value}" 不唯一（与记录 "${existing}" 重复）`, 'UNIQUE_FIELD_DUPLICATE', field, record.id || record.oid || record.code);
            }
            else {
 ...
 * 验证表完整性
 */
function verifyTableIntegrity(tableName, options) {
    ...
    if (!verifyTableExists(tableName)) {
        addProblem(problems, 'error', tableName, `表 "${tableName}" 不存在`, 'TABLE_NOT_EXISTS');
 ...
 */
function verifySeedSourceQuantityConservation() {
    const problems = [];
    ...
    records.forEach((record) => {
        ...
        if (used + remaining !== total) {
            inconsistentCount++;
            addProblem(problems, 'error', 'seed_sources', `种源 "${record.source_code}" 数量不守恒：使用量(${used}) + 剩余量(${remaining}) = ${used + remaining}，不等于总量(${total})`, 'QUANTITY_NOT_CONSERVED', 'quantity', record.id);
        }
    });
    return {
        checkName: '种源数量守恒验证',
 ...
 */
function verifyPlantingQuantityConservation() {
    const problems = [];
        ...
        if (planting.source_type === 'seedling' && planting.source_id) {
            ...
            if (seedling) {
                ...
                if (plantedQty > seedlingQty) {
                    inconsistentCount++;
                    addProblem(problems, 'warning', 'plantings', `种植记录 "${planting.planting_code}" 的已种植面积(${plantedQty})超过育苗数量(${seedlingQty})`, 'QUANTITY_EXCEEDS_SOURCE', 'planted_quantity', planting.id);
                }
            }
        }
    });
 ...
 * 验证采收数量合理性
 */
function verifyHarvestQuantity合理性() {
    const problems = [];
    ...
    harvests.forEach((harvest) => {
        ...
        if (qty <= 0) {
            invalidCount++;
            addProblem(problems, 'error', 'harvest_records', `采收记录 "${harvest.harvest_code}" 的采收数量无效(${qty})`, 'HARVEST_QUANTITY_INVALID', 'harvest_quantity', harvest.id);
        }
        // 验证金额计算一致性（允许浮点误差）
        if (price > 0 && total > 0) {
            ...
            if (diff > 0.01) {
                addProblem(problems, 'warning', 'harvest_records', `采收记录 "${harvest.harvest_code}" 的金额可能不一致：数量(${qty}) × 单价(${price}) = ${calculatedTotal}，记录总金额为 ${total}`, 'HARVEST_AMOUNT_MISMATCH', 'total_amount', harvest.id);
            }
        }
    });
    return {
...
function verifyInventoryConsistency() {
    ...
    // 建立采收记录索引
    const harvestIds = new Set(harvestRecords.map((h) => h.id));
    let invalidCount = 0;
    inventories.forEach((inv) => {
        // 验证仓库ID有效性
        if (inv.warehouse_id && !warehouseIds.has(inv.warehouse_id)) {
            invalidCount++;
            addProblem(problems, 'error', 'inventory', `库存记录 "${inv.product_code}" 的仓库ID "${inv.warehouse_id}" 不存在`, 'INVENTORY_WAREHOUSE_INVALID', 'warehouse_id', inv.id);
        }
        // 验证采收记录ID有效性
        if (inv.harvest_record_id && !harvestIds.has(inv.harvest_record_id)) {
            addProblem(problems, 'warning', 'inventory', `库存记录 "${inv.product_code}" 的采收记录ID "${inv.harvest_record_id}" 不存在`, 'INVENTORY_HARVEST_INVALID', 'harvest_record_id', inv.id);
        }
        // 验证数量非负
        if (inv.quantity < 0) {
            invalidCount++;
            addProblem(problems, 'error', 'inventory', `库存记录 "${inv.product_code}" 的数量为负数(${inv.quantity})`, 'INVENTORY_QUANTITY_NEGATIVE', 'quantity', inv.id);
        }
    });
    return {
        checkName: '库存一致性验证',
 ...
 * 验证用户与创建者关联
 */
function verifyUserConsistency() {
    const problems = [];
    ...
    tablesWithCreateBy.forEach((tableName) => {
        ...
        records.forEach((record) => {
            ...
            if (record.create_by_id && !userIds.has(record.create_by_id)) {
                orphanCount++;
                addProblem(problems, 'warning', tableName, `表 "${tableName}" 中记录 "${record.id}" 的创建者ID "${record.create_by_id}" 不存在于用户表中`, 'CREATE_BY_ORPHANED', 'create_by_id', record.id);
            }
        });
    });
    return {
 ...
 * 验证采收记录的完整溯源链
 */
function verifyHarvestTraceability() {
    const results = [];
    const harvests = getAllRecords('harvest_records');
    const plantings = getAllRecords('plantings');
    const seedlings = getAllRecords('seedlings');
    ...
    // 建立索引
    const plantingMap = new Map();
    plantings.forEach((p) => plantingMap.set(p.id, p));
    const seedlingMap = new Map();
    seedlings.forEach((s) => seedlingMap.set(s.id, s));
    const seedSourceMap = new Map();
    seedSources.forEach((s) => seedSourceMap.set(s.id, s));
    harvests.forEach((harvest) => {
        const chain = [];
        const problems = [];
        // Level 0: 采收记录本身
        ...
        if (harvest.source_type === 'planting' && harvest.source_id) {
            ...
            if (!planting) {
                addProblem(problems, 'error', 'harvest_records', `采收记录 "${harvest.harvest_code}" 关联的种植记录 "${harvest.source_id}" 不存在`, 'TRACE_PLANTING_NOT_FOUND', 'source_id', harvest.id);
            }
            else {
                // Level 2: 育苗记录
                if (planting.source_type === 'seedling' && planting.source_id) {
                    ...
                    if (!seedling) {
                        addProblem(problems, 'error', 'plantings', `种植记录 "${planting.planting_code}" 关联的育苗记录 "${planting.source_id}" 不存在`, 'TRACE_SEEDLING_NOT_FOUND', 'source_id', planting.id);
                    }
                    else {
                        // Level 3: 种源记录
                        if (seedling.source_id) {
                            ...
                            if (!seedSource) {
                                addProblem(problems, 'error', 'seedlings', `育苗记录 "${seedling.seedling_code}" 关联的种源记录 "${seedling.source_id}" 不存在`, 'TRACE_SEED_SOURCE_NOT_FOUND', 'source_id', seedling.id);
                            }
                        }
                    }
                }
 ...
 * 验证种植记录的完整溯源链
 */
function verifyPlantingTraceability() {
    const results = [];
    const plantings = getAllRecords('plantings');
    const seedlings = getAllRecords('seedlings');
    const seedSources = getAllRecords('seed_sources');
 ...
 * 验证溯源模块（种源→育苗→种植→采收）
 */
function verifyTraceabilityModule() {
 ...
 * 验证库存模块
 */
function verifyInventoryModule() {
...
function verifySystemModule() {
    ...
    tables.forEach((table) => {
        if (!verifyTableExists(table.name)) {
            addProblem(problems, 'error', table.name, `表 "${table.name}" 不存在`, 'TABLE_NOT_EXISTS');
            totalErrors++;
            return;
        }
 ...
 * 验证字典模块
 */
function verifyDictionaryModule() {
    ...
    dictionaries.forEach((dict) => {
        if (!categoryCodes.has(dict.category_code)) {
            addProblem(problems, 'error', 'dictionaries', `字典项 "${dict.dict_label}" 的分类编码 "${dict.category_code}" 不存在`, 'DICT_CATEGORY_NOT_FOUND', 'category_code', dict.id);
        }
    });
    // 验证作物品种表
 ...
 * 验证指定模块
 */
export function verifyModule(moduleName) {
