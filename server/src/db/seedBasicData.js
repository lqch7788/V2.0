...
export function seedCodeRuleCategories() {
    ...
    const insertCategory = (code, name, nameEn, parentCode, level, ruleType, sortOrder) => {
        ...
        const existing = db.exec(`SELECT id FROM material_code_categories WHERE code = ? AND parent_code = ? AND rule_type = ? AND status = 'active'`, [code, parentCode, ruleType]);
        if (existing.length > 0 && existing[0].values.length > 0)
            return;
        const id = `${ruleType === 'supplier' ? 'SCC' : 'MCC'}${Date.now()}_${parentCode}_${code}`;
        db.run(`INSERT INTO material_code_categories (id, code, name, name_en, parent_code, level, rule_type, sort_order, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?)`, [id, code, name, nameEn, parentCode, level, ruleType, sortOrder, now, now]);
        added++;
    };
    // ========== 供应商编码规则（2级：big → mid）==========
...
export function seedIndicatorEvaluations() {
    ...
    if (inserted > 0)
        console.log(`已导入 ${inserted} 条指标评估数据`);
}
