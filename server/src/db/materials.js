 ...
 * 根据ID获取入库记录
 */
export function getInboundRecordById(id) {
    const db = getDatabase();
 ...
 * 更新入库记录
 */
export function updateInboundRecord(id, updates) {
    const db = getDatabase();
 ...
 */
export function syncInboundToMaterials(materials) {
    const db = getDatabase();
    for (const m of materials) {
        ...
        const existing = db.exec('SELECT id, quantity FROM materials WHERE code = ? AND batchNo = ?', [m.code, batchNo]);
        if (existing.length > 0 && existing[0].values.length > 0) {
            // 已有同批次：累加数量 + 恢复启用状态（防止之前因用完被标记）
            const oldQty = existing[0].values[0][1];
            const newQty = oldQty + (Number(m.quantity) || 0);
            db.run('UPDATE materials SET quantity = ?, lastUpdateTime = ?, dataStatus = ? WHERE code = ? AND batchNo = ?', [newQty, new Date().toISOString(), '启用', m.code, batchNo]);
        }
        else {
            // 新批次：新增物料记录
            db.run(`
