...
class MigrationExecutor {
     ...
     * 迁移单个表
     */
    async migrateTable(storageKey, tableName, transform) {
                ...
                for (const record of batch) {
                    try {
                        ...
                        if (this.config.dryRun) {
                            ...
                            // 检查是否已存在
                            const existing = this.db.prepare(`SELECT id FROM ${tableName} WHERE id = ?`).get(transformed.id);
                            ...
                            if (this.config.forceOverwrite && existing) {
                                ...
                                this.db.prepare(`UPDATE ${tableName} SET ${setClause} WHERE id = ?`).run(...values, transformed.id);
                            }
                            else {
