...
describe('Approval Workflow Utils', () => {
  ...
  describe('审批查询', () => {
    it('应该按申请人过滤', () => {
      const filterByApplicant = (applicant) => {
  ...
  describe('审批通知', () => {
    it('应该生成待审批通知', () => {
      const generateNotification = (record) => {
        return {
    ...
    it('应该生成审批结果通知', () => {
      const generateResultNotification = (record) => {
        return {
  ...
  describe('批量审批', () => {
    it('应该处理批量审批结果', () => {
      const processBatchResult = (ids, action) => {
        return ids.map((id) => ({
          id,
          action,
          success: true,
    ...
    it('应该处理部分失败', () => {
      const processWithPartialFailure = (ids) => {
        return ids.map((id) => ({
          id,
          success: id !== 2,
