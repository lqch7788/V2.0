 ...
 * 验证审批动作是否有效
 */
function isValidAction(action, currentStatus) {
  if (currentStatus === APPROVAL_STATUS.PENDING) {
    return [APPROVAL_ACTION.APPROVE, APPROVAL_ACTION.REJECT, APPROVAL_ACTION.PARTIALLY_APPROVE, APPROVAL_ACTION.CANCEL].includes(action);
  }
