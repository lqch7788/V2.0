 ...
 */
router.post('/batch-print', (req, res) => {
    try {
 ...
 * 状态为已完成且未定植的记录
 */
router.get('/transplant-ready', (req, res) => {
    try {
 ...
 */
router.get('/label-number', (req, res) => {
    try {
...
router.post('/', (req, res) => {
    try {
        const { id, seedling_code, source_id, source_name, crop_name, crop_variety, seedling_type, greenhouse_name, area_name, seedling_date, expected_finish_date, seedling_quantity, survival_quantity, survival_rate, status, seedling_status, remarks, create_by, work_hours } = req.body;
...
router.post('/:id/daily-records', (req, res) => {
    try {
        const { id } = req.params;
        const { record_date, crop_name, crop_variety, greenhouse_name, quantity, unit, data, remarks, create_by } = req.body;
        // 验证育苗记录是否存在
        const db = getDatabase();
        const stmt = db.prepare('SELECT * FROM seedlings WHERE id = ?');
        stmt.bind([id]);
 ...
 */
router.post('/:id/transplant-records', (req, res) => {
    try {
        const { id } = req.params;
        const { crop_name, crop_variety, greenhouse_name, area_name, from_location, to_location, transplant_date, transplant_quantity, survival_quantity, survival_rate, operator_id, operator_name, status, remarks, data, create_by } = req.body;
        // 验证育苗记录是否存在
        const db = getDatabase();
        const stmt = db.prepare('SELECT * FROM seedlings WHERE id = ?');
        stmt.bind([id]);
 ...
 */
router.put('/:id/transplant-records/:recordId/status', (req, res) => {
    try {
 ...
 */
router.get('/:id/transplant-records', (req, res) => {
    try {
        const { id } = req.params;
        const { page = 1, limit = 50 } = req.query;
        const db = getDatabase();
...
router.post('/:id/print', (req, res) => {
    try {
        const { id } = req.params;
        const { print_type, print_title, printer_name, paper_size, copies, print_status, error_message, data, create_by } = req.body;
        // 验证育苗记录是否存在
        const db = getDatabase();
        const stmt = db.prepare('SELECT * FROM seedlings WHERE id = ?');
        stmt.bind([id]);
 ...
 */
router.get('/:id/available-count', (req, res) => {
    try {
        const { id } = req.params;
        const db = getDatabase();
        const stmt = db.prepare('SELECT * FROM seedlings WHERE id = ?');
 ...
 */
router.post('/:id/increase-planted', (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.get('/:id/all-label-numbers', (req, res) => {
    try {
        const { id } = req.params;
        const db = getDatabase();
        const stmt = db.prepare('SELECT * FROM seedlings WHERE id = ?');
 ...
 */
router.get('/:id/transplant-history', (req, res) => {
    try {
        const { id } = req.params;
        const db = getDatabase();
        // 从定植记录表获取所有相关的栽种履历
 ...
 */
router.get('/:id/transplant-history/:labelNumber', (req, res) => {
    try {
 ...
 */
router.post('/:id/transplant-history/:labelNumber', (req, res) => {
    try {
        const { id, labelNumber } = req.params;
        const { to_area, to_location, operator_id, operator_name, remarks, create_by } = req.body;
        const db = getDatabase();
        // 验证育苗记录是否存在
        const stmt = db.prepare('SELECT * FROM seedlings WHERE id = ?');
        stmt.bind([id]);
 ...
 */
router.put('/:id/transplant-history/:labelNumber/status', (req, res) => {
    try {
        const { id, labelNumber } = req.params;
        const { status } = req.body;
        if (!status) {
