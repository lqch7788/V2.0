...
function transformInspectionRecord(db) {
    ...
    if (typeof feedbackUsers === 'string') {
        try {
            feedbackUsers = JSON.parse(feedbackUsers);
        }
        catch {
            feedbackUsers = [];
        }
    }
    if (!Array.isArray(feedbackUsers))
        feedbackUsers = [];
    return {
        ...db,
        // 确保必要字段存在
...
router.post('/', (req, res) => {
    try {
        const { id, record_code, inspection_type, inspector_id, inspector_name, greenhouse_name, check_date, check_time, check_result, issue_severity, issue_text, images, status, feedbackUsers } = req.body;
