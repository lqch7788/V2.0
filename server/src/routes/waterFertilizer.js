...
router.get('/', (req, res) => {
    try {
        ...
        const params = [];
        if (status) {
            conditions.push('wf.status = ?');
            params.push(status);
        }
        if (partition_oid) {
            conditions.push('wf.partition_oid = ?');
            params.push(partition_oid);
        }
...
router.post('/', (req, res) => {
    try {
        const db = getDatabase();
        const { partition_oid, device_oid, device_code, machine_addr, mac_addr, start_time, end_time, interval_value, interval_unit, mix_ratio_a, mix_ratio_b, mix_ratio_c, description } = req.body;
        if (!partition_oid)
            return res.status(400).json({ success: false, error: '分区不能为空' });
...
router.put('/:oid', (req, res) => {
    try {
        const db = getDatabase();
        const { partition_oid, device_oid, device_code, machine_addr, mac_addr, start_time, end_time, interval_value, interval_unit, mix_ratio_a, mix_ratio_b, mix_ratio_c, description, status } = req.body;
        const now = new Date().toISOString();
        db.run(`
