...
router.get('/', (req, res) => {
    try {
        ...
        const parentId = parseInt(req.query.parent_id, 10) || 0;
        const result = db.exec(`SELECT id, name, parent_id, level FROM region_data WHERE parent_id = ? ORDER BY id`, [parentId]);
...
router.get('/search', (req, res) => {
    try {
        ...
        const result = db.exec(`SELECT id, name, parent_id, level FROM region_data WHERE ${conditions.join(' AND ')} LIMIT 50`, params);
        const items = result.length > 0 && result[0].values
            ? result[0].values.map((row) => ({
                id: row[0],
                name: row[1],
