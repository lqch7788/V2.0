...
router.get('/', (req, res) => {
    ...
    const parentId = parseInt(req.query.parent_id) || 0;
...
router.get('/search', (req, res) => {
    ...
    if (!keyword || keyword.length < 1) {
      res.status(400).json({ success, error: '请提供搜索关键字' });
      return;
    }
...
router.post('/init', (req, res) => {
    ...
    // 检查是否已有完整数据（含城市级）
    const existing = db.exec(`SELECT COUNT(*)`);
    const count = existing[0]?.values[0]?.[0] ?? 0;
    if (Number(count) > 40) {
      res.json({ success, data: { message: '行政区划数据已存在，跳过初始化', count } });
      return;
    }
    ...
    res.json({
      success,
      data: { message: '行政区划种子数据初始化完成', provinces, cities, total: nextId - 1 },
    });
  } catch (error) {
    res.status(500).json({ success, error: (error).message });
  }
