...
router.post('/', (req, res) => {
    try {
        const { id, worker_id, worker_name, leave_type, start_date, end_date, days, reason, status, department_id, department_name, remarks } = req.body;
 ...
 */
router.get('/quotas/list', (req, res) => {
    try {
 ...
 */
router.get('/quotas/summary', (req, res) => {
    try {
        const { worker_id, year } = req.query;
        const db = getDatabase();
        if (!worker_id || !year) {
...
router.post('/quotas', (req, res) => {
    try {
        const { id, worker_id, worker_name, year, leave_category, total_days, department_id, department_name, remarks } = req.body;
 ...
 */
router.post('/quotas/freeze', (req, res) => {
    try {
        ...
        const db = getDatabase();
        // 检查并更新额度
        const checkStmt = db.prepare('SELECT * FROM leave_quotas WHERE worker_id = ? AND year = ? AND leave_category = ?');
 ...
 */
router.post('/quotas/release', (req, res) => {
    try {
        const { worker_id, leave_type, days, year } = req.body;
        if (!worker_id || !leave_type || days === undefined) {
            return res.status(400).json({ success: false, error: '缺少必要参数' });
 ...
 */
router.post('/quotas/deduct', (req, res) => {
    try {
        const { worker_id, leave_type, days, year } = req.body;
        if (!worker_id || !leave_type || days === undefined) {
            return res.status(400).json({ success: false, error: '缺少必要参数' });
