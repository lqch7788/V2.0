...
router.get('/', (req, res) => {
    ...
    res.status(500).json({ success, error: '获取分区列表失败' });
  }
});
...
router.get('/:oid', (req, res) => {
    ...
    if (result.length === 0 || result[0].values.length === 0) {
      return res.status(404).json({ success, error: '分区不存在' });
    }
    const columns = result[0].columns;
    const record= {};
    ...
    res.status(500).json({ success, error: '获取分区详情失败' });
  }
});
...
router.get('/tree/all', (req, res) => {
    ...
    res.status(500).json({ success, error: '获取分区树失败' });
  }
});
...
router.post('/', (req, res) => {
    ...
    if (!name) return res.status(400).json({ success, error: '分区名称不能为空' });
    ...
    res.status(500).json({ success, error: '创建分区失败' });
  }
});
...
router.put('/:oid', (req, res) => {
    ...
    res.status(500).json({ success, error: '更新分区失败' });
  }
});
...
router.delete('/:oid', (req, res) => {
    ...
    const cascadeDelete = (oid) => {
      ...
      if (children.length > 0 && children[0].values.length > 0) {
        children[0].values.forEach(row => cascadeDelete(row[0]));
      }
    };
    cascadeDelete(req.params.oid);
    res.json({ success, message: '分区已删除' });
  } catch (error) {
    console.error('删除分区失败:', error);
    res.status(500).json({ success, error: '删除分区失败' });
  }
});
