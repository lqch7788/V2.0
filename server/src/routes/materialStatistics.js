...
// 物料分类7色配色方案
const CATEGORY_COLORS, { gradient: string[]; solid= {
...
router.get('/', (_req, res) => {
      ...
      for (const m of mats) {
        ...
        if (!materialMap.has(key)) {
          materialMap.set(key, {
            ...
            production_plan_batch_code: (rec).production_batch_code || '',
            requisition_department: (rec).department_name || '',
            usage_area: (rec).plant_area || '',
            requisitioner: (rec).applicant_name || '',
            requisition_time: String((rec).apply_date || ''),
            requisition_count,
            total_quantity,
            actual_quantity,
            total_amount,
            main_warehouse: (rec).warehouse_name || '',
          });
        }
    ...
    for (const rec of records) {
      const applyDate = String((rec).apply_date || '');
      if (!applyDate) continue;
      const parts = applyDate.split('-');
      const year = parts[0];
      const month = parts[1];
      if (!year || !month) continue;
      const dept = (rec).department_name || '';
      const key = `${year}-${month}-${dept}`;
      if (!monthlyMap.has(key)) {
        const mats = (rec.materials) || [];
        const totalQty = mats.reduce((s, m) => s + Number(m.requestedQuantity || m.quantity || 0), 0);
        const actualQty = mats.reduce((s, m) => s + Number(m.actualQuantity || m.requestedQuantity || m.quantity || 0), 0);
        const totalAmt = mats.reduce((s, m) => s + Number(m.unitPrice || 0) * Number(m.requestedQuantity || m.quantity || 0), 0);
        monthlyMap.set(key, {
          year, month,
          department,
          requisition_count,
          material_types: new Set(mats.map((m) => m.materialCode || m.code)),
          total_quantity,
          actual_quantity,
        ...
        const mats = (rec.materials) || [];
        const matTypes = new Set(mats.map((m) => m.materialCode || m.code));
        entry.requisition_count += 1;
        entry.material_types = new Set([...entry.material_types, ...matTypes]);
        entry.total_quantity += mats.reduce((s, m) => s + Number(m.requestedQuantity || m.quantity || 0), 0);
        entry.actual_quantity += mats.reduce((s, m) => s + Number(m.actualQuantity || m.requestedQuantity || m.quantity || 0), 0);
        entry.total_amount += mats.reduce((s, m) => s + Number(m.unitPrice || 0) * Number(m.requestedQuantity || m.quantity || 0), 0);
        entry.difference_rate = entry.total_quantity > 0 ? ((entry.actual_quantity - entry.total_quantity) / entry.total_quantity * 100) : 0;
      }
    }
    const monthlyStatistics = Array.from(monthlyMap.values()).map((e) => {
    ...
    for (const rec of records) {
      ...
      if (!trendMap.has(ym)) {
        trendMap.set(ym, { month, 生产投入: 0, 设施装备: 0, 作业支持: 0, 采后流通: 0, 数字管理: 0, 能源耗材: 0, 其他: 0 });
      }
    ...
    res.status(500).json({ success, error: '获取领料统计失败' });
  }
});
