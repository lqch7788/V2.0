...
router.get('/', (req, res) => {
    ...
    res.status(500).json({ success, error: '获取种植设置失败' });
  }
});
...
router.post('/', (req, res) => {
    ...
    if (!setting_key) return res.status(400).json({ success, error: '设置键不能为空' });
    ...
    res.status(500).json({ success, error: '创建种植设置失败' });
  }
});
...
router.put('/:oid', (req, res) => {
    ...
    if (result.length === 0 || result[0].values.length === 0) {
      return res.status(404).json({ success, error: '设置不存在' });
    }
    const columns = result[0].columns;
    const record= {};
    columns.forEach((col, i) => { record[col] = result[0].values[0][i]; });
    ...
    res.status(500).json({ success, error: '更新种植设置失败' });
  }
});
...
router.delete('/:oid', (req, res) => {
    ...
    res.json({ success, message: '设置已删除' });
  } catch (error) {
    console.error('删除种植设置失败:', error);
    res.status(500).json({ success, error: '删除种植设置失败' });
  }
});
