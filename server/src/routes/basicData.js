...
import { exportBasicData } from '../db/seedBasicData.js';
const router = Router();
/**
 * 获取所有部门
...
router.post('/greenhouses', (req, res) => {
    try {
        const db = getDatabase();
        const { name, code, greenhouseType, area, location, baseOid, baseName, companyId, companyName, lng, lat, crop, growthDay, manager, phone, soilType, ph, intro, greenhouseCount, fieldArea } = req.body;
        if (!name || !code) {
...
router.put('/greenhouses/:id', (req, res) => {
    try {
        const db = getDatabase();
        const { id } = req.params;
        const { name, code, greenhouseType, area, location, baseOid, baseName, companyId, companyName, lng, lat, crop, growthDay, manager, phone, soilType, ph, intro, greenhouseCount, fieldArea, status } = req.body;
        const now = new Date().toISOString();
        db.run(`
      UPDATE greenhouses
      SET name = COALESCE(?, name),
...
router.post('/material-types', (req, res) => {
    try {
        ...
        const now = new Date().toISOString();
        db.run(`INSERT INTO material_types (oid, type_code, type_name, category, default_unit, default_price, specifications, description, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [oid, typeCode, typeName, category || null, defaultUnit || null, defaultPrice || 0, specifications || null, description || null, status || 'active', now, now]);
...
router.put('/material-types/:id', (req, res) => {
    try {
        ...
        if (setClauses.length === 0)
            return res.status(400).json({ success: false, error: '无有效字段' });
