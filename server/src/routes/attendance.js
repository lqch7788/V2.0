 ...
 * GET /api/attendance
 */
router.get('/', (req, res) => { start_date, end_date, dept, keyword, page = '1', limit = '100' } = req.query;
    ...
    if (dept && dept !== '全部') { countSql += ' AND dept = ?'; countParams.push(dept); }
    if (keyword) { countSql += ' AND (name LIKE ? OR worker_id LIKE ?)'; countParams.push(`%${keyword}%`, `%${keyword}%`); }
    ...
    res.status(500).json({ success, error: '获取考勤列表失败' });
  }
});
...
router.get('/:id', (req, res) => { id } = req.params;
    ...
    if (result.length === 0 || result[0].values.length === 0) {
      res.status(404).json({ success, error: '考勤记录不存在' });
      return;
    }
    ...
    res.status(500).json({ success, error: '获取考勤详情失败' });
  }
});
 ...
 */
router.post('/batch', (req, res) => { records } = req.body;
    if (!Array.isArray(records) || records.length === 0) {
      res.status(400).json({ success, error: '请提供考勤数据数组' });
      return;
    }
    ...
    res.status(500).json({ success, error: '批量创建考勤记录失败' });
  }
});
...
router.put('/:id', (req, res) => { id } = req.params;
    ...
    if (updates.worker_id !== undefined) { fields.push('worker_id = ?'); values.push(updates.worker_id); }
    if (updates.name !== undefined) { fields.push('name = ?'); values.push(updates.name); }
    if (updates.dept !== undefined) { fields.push('dept = ?'); values.push(updates.dept); }
    ...
    if (updates.status_class !== undefined) { fields.push('status_class = ?'); values.push(updates.status_class); }
    if (updates.task_id !== undefined) { fields.push('task_id = ?'); values.push(updates.task_id); }
    if (updates.batch_id !== undefined) { fields.push('batch_id = ?'); values.push(updates.batch_id); }
    ...
    res.status(500).json({ success, error: '更新考勤记录失败' });
  }
});
...
router.delete('/batch', (req, res) => { ids } = req.body;
    ...
    res.status(500).json({ success, error: '批量删除考勤记录失败' });
  }
});
