...
router.get('/', (req, res) => {
    ...
    if (module && module !== 'all') countBindings.push(module);
    if (level && level !== 'all') countBindings.push(level);
    if (start_date) countBindings.push(start_date);
    if (end_date) countBindings.push(end_date);
    ...
    res.status(500).json({ success, error: '获取操作日志失败' });
  }
});
...
router.get('/:id', (req, res) => {
    ...
    if (!log) {
      return res.status(404).json({ success, error: '日志不存在' });
    }

    res.json({ success: true, data: logs);
  } catch (error) {
    console.error('获取日志详情失败:', error);
    res.status(500).json({ success, error: '获取日志详情失败' });
  }
});
...
router.post('/', (req, res) => {
    ...
    res.status(500).json({ success, error: '创建操作日志失败' });
  }
});
...
router.get('/stats/summary', (req, res) => {
    ...
    // 获取总数
    const totalStmt = db.prepare('SELECT COUNT(*)');
    if (totalStmt.step()) {
    ...
    const todayStmt = db.prepare('SELECT COUNT(*)(created_at) = ?');
    todayStmt.bind([today]);
    if (todayStmt.step()) {
      const row = todayStmt.getAsObject();
      stats.today = (row.count) || 0;
    }
    todayStmt.free();
    ...
    res.status(500).json({ success, error: '获取统计信息失败' });
  }
});
...
router.delete('/:id', (req, res) => {
    ...
    res.json({ success, message: '日志已删除' });
  } catch (error) {
    console.error('删除日志失败:', error);
    res.status(500).json({ success, error: '删除日志失败' });
  }
});
