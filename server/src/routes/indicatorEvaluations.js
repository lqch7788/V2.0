...
router.get('/', (_req, res) => {
    try {
        ...
        res.status(500).json({ success: false, error: '获取评估数据失败' });
    }
});
/**
 * POST / — 创建评估记录
...
router.post('/', (req, res) => {
    try {
        ...
        if (!name) {
            return res.status(400).json({ success: false, error: '基地名称不能为空' });
        }
    ...
    `, [
        ...
        res.status(500).json({ success: false, error: '创建评估记录失败' });
    }
});
/**
 * PUT /:id — 更新评估记录
...
router.put('/:id', (req, res) => {
    try {
        ...
        checkStmt.free();
        if (!exists) {
            return res.status(404).json({ success: false, error: '评估记录不存在' });
        }
        ...
        if (productionScore !== undefined) {
            fields.push('production_score = ?');
            values.push(Number(productionScore));
        }
        if (qualityScore !== undefined) {
            fields.push('quality_score = ?');
            values.push(Number(qualityScore));
        }
        if (costScore !== undefined) {
            fields.push('cost_score = ?');
            values.push(Number(costScore));
        }
        if (efficiencyScore !== undefined) {
            fields.push('efficiency_score = ?');
            values.push(Number(efficiencyScore));
        }
        if (totalScore !== undefined) {
            fields.push('total_score = ?');
            values.push(Number(totalScore));
        }
        if (rank !== undefined) {
            fields.push('rank = ?');
            values.push(Number(rank));
        }
        if (fields.length === 0) {
        ...
        res.status(500).json({ success: false, error: '更新评估记录失败' });
    }
});
/**
 * DELETE /:id — 删除评估记录
...
router.delete('/:id', (req, res) => {
    try {
        ...
        saveDatabase();
        res.json({ success: true, message: '评估记录删除成功' });
    }
    catch (error) {
        console.error('删除评估记录失败:', error);
        res.status(500).json({ success: false, error: '删除评估记录失败' });
    }
});
