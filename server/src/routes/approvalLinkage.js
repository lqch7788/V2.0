 ...
 * 更新物料申请状态
 */
function updateMaterialRequest(db, id, status, approvalCode, extra?: Record<string, unknown>){
    const now = new Date().toISOString();
    db.run(`
 ...
 * 更新采购计划状态
 */
function updatePurchasePlan(db, id, status, approvalCode, extra?: Record<string, unknown>){
    const now = new Date().toISOString();
 ...
 */
function updateProductionPlan(db, id, status, approvalCode, extra?: Record<string, unknown>){
    const now = new Date().toISOString();
    // 映射审批状态到生产计划状态
 ...
 * 更新技术方案状态
 */
function updateTechSolution(db, id, status, approvalCode, extra?: Record<string, unknown>){
    const now = new Date().toISOString();
    // 映射审批状态到技术方案状态
 ...
 * 更新农事任务状态
 */
function updateFarmTask(db, id, status, approvalCode, extra?: Record<string, unknown>){
    const now = new Date().toISOString();
    db.run(`
 ...
 * 更新采收记录状态
 */
function updateHarvestRecord(db, id, status, approvalCode, extra?: Record<string, unknown>){
    const now = new Date().toISOString();
    db.run(`
 ...
 * 更新订单状态
 */
function updateCropOrder(db, id, status, approvalCode, extra?: Record<string, unknown>){
    const now = new Date().toISOString();
    db.run(`
 ...
 * 更新巡查记录状态
 */
function updateInspection(db, id, status, approvalCode, extra?: Record<string, unknown>){
    const now = new Date().toISOString();
    db.run(`
 ...
 * 更新问题记录状态
 */
function updateProblem(db, id, status, approvalCode, extra?: Record<string, unknown>){
    const now = new Date().toISOString();
    db.run(`
      UPDATE problems SET
        status = ?,
 ...
 * 更新人工记录状态（请假/加班/调薪等）
 */
function updateLaborRecord(db, id, status, approvalCode, extra?: Record<string, unknown>){
    const now = new Date().toISOString();
    db.run(`
 ...
 * 创建请假记录（如果表存在）
 */
function createLeaveRecord(db, data){
    // 检查表是否存在
 ...
 * 创建加班记录
 */
function createOvertimeRecord(db, data){
 ...
 * 更新员工状态
 */
function updateEmployee(db, id, status, approvalCode, extra?: Record<string, unknown>){
    const now = new Date().toISOString();
 ...
 * 更新预算状态
 */
function updateBudget(db, id, status, approvalCode, extra?: Record<string, unknown>){
 ...
 * 更新指标状态
 */
function updateIndicator(db, id, status, approvalCode, extra?: Record<string, unknown>){
 ...
 * 更新公告状态
 */
function updateAnnouncement(db, id, status, approvalCode, extra?: Record<string, unknown>){
...
){ success: boolean; message= new Date().toISOString();
  let status = 'pending';
  ...
  switch (businessType) {
    ...
    case 'material':
      if (updateMaterialRequest(db, requestId, status, approvalCode, extra)) {
        return { success, message: '物料申请状态已更新' };
      }
      break;

    case 'purchase':
      if (updatePurchasePlan(db, requestId, status, approvalCode, extra)) {
        return { success, message: '采购计划状态已更新' };
      }
      break;
    ...
    case 'material_inbound':
      ...
      try {
        ...
        return { success, message: '物料入库状态已更新' };
      } catch (e) {
        console.error('更新物料入库失败:', e);
        return { success, message: '数据库更新失败: ' + (e instanceof Error ? e.message : String(e)) };
      }
      break;
    ...
    case 'material_transfer':
      ...
      try {
        db.run(`
          ...
          WHERE id = ?
        `, [status, approvalCode, now, now, requestId]);
        return { success, message: '库存调拨状态已更新' };
      } catch (e) {
    ...
    case 'seed_source_inbound':
      ...
      try {
        db.run(`
          ...
          WHERE id = ?
        `, [status, approvalCode, now, now, requestId]);
        return { success, message: '种源入库状态已更新' };
      } catch (e) {
    ...
    case 'seedling_plan':
      ...
      try {
        db.run(`
          ...
          WHERE id = ?
        `, [status, approvalCode, now, now, requestId]);
        return { success, message: '育苗计划状态已更新' };
      } catch (e) {
    ...
    case 'planting_plan':
      ...
      try {
        db.run(`
          ...
          WHERE id = ?
        `, [status, approvalCode, now, now, requestId]);
        return { success, message: '种植计划状态已更新' };
      } catch (e) {
    ...
    case 'order_change':
      if (updateCropOrder(db, requestId, finalStatus, approvalCode, extra)) {
        return { success, message: '订单状态已更新' };
      }
      break;
    ...
    case 'production':
      ...
      if (updateProductionPlan(db, requestId, action, approvalCode, extra)) {
        return { success, message: '生产计划状态已更新' };
      }
      break;
    ...
    case 'production_batch':
      ...
      try {
        db.run(`
          ...
          WHERE id = ?
        `, [status, approvalCode, now, now, requestId]);
        return { success, message: '生产批次状态已更新' };
      } catch (e) {
    ...
    case 'batch_void':
      ...
      try {
        ...
        return { success, message: '批次状态已更新' };
      } catch (e) {
    ...
    case 'tech_solution':
      ...
      if (updateTechSolution(db, requestId, action, approvalCode, extra)) {
        return { success, message: '技术方案状态已更新' };
      }
      break;
    ...
    case 'harvest':
      if (updateHarvestRecord(db, requestId, status, approvalCode, extra)) {
        return { success, message: '采收申请状态已更新' };
      }
      break;
    ...
    case 'seed_source':
      try {
        db.run(`
          ...
          WHERE id = ?
        `, [status, approvalCode, now, now, requestId]);
        return { success, message: '种源补录状态已更新' };
      } catch (e) {
    ...
    case 'seedling':
      try {
        db.run(`
          ...
          WHERE id = ?
        `, [status, approvalCode, now, now, requestId]);
        return { success, message: '育苗补录状态已更新' };
      } catch (e) {
    ...
    case 'crop_storage':
      try {
        db.run(`
          ...
          WHERE id = ?
        `, [status, approvalCode, now, now, requestId]);
        return { success, message: '作物入库补录状态已更新' };
      } catch (e) {
    ...
    case 'budget_adjust':
      if (updateBudget(db, requestId, status, approvalCode, extra)) {
        return { success, message: '预算状态已更新' };
      }
      break;
    ...
    case 'leave':
      ...
      try {
        ...
        if (tableCheck.step()) {
          ...
          return { success, message: '请假记录状态已更新' };
        }
        tableCheck.free();
      } catch (e) {
    ...
    case 'overtime':
      ...
      try {
        ...
        if (tableCheck.step()) {
          ...
          return { success, message: '加班记录状态已更新' };
        }
        tableCheck.free();
      } catch (e) {
    ...
    case 'resign':
      if (updateEmployee(db, requestId, 'resigned', approvalCode, extra)) {
        ...
        return { success, message: '员工离职状态已更新' };
      }
      break;
    ...
    case 'recruitment':
      ...
      try {
        db.run(`
          ...
          WHERE id = ?
        `, [status, approvalCode, now, now, requestId]);
        return { success, message: '招聘状态已更新' };
      } catch (e) {
    ...
    case 'contract_renewal':
      ...
      try {
        ...
        if (tableCheck.step()) {
          ...
          return { success, message: '合同续签状态已更新' };
        }
        tableCheck.free();
      } catch (e) {
      ...
      try {
        ...
        if (tableCheck.step()) {
          ...
          db.run(`
            ...
            WHERE id = ?
          `, [status, approvalCode, now, now, requestId]);
          return { success, message: '工资预算状态已更新' };
        }
        tableCheck.free();
      } catch (e) {
    ...
    case 'transfer':
      if (updateEmployee(db, requestId, 'transferred', approvalCode, extra)) {
        return { success, message: '员工转岗状态已更新' };
      }
      break;
    ...
    case 'return':
      try {
        db.run(`
          ...
          WHERE id = ?
        `, [status, approvalCode, now, now, requestId]);
        return { success, message: '退料单状态已更新' };
      } catch (e) {
    ...
    default:
      ...
      return { success, message: `未知的业务类型: ${businessType}` };
  }

  return { success, message: '更新失败' };
}
...
router.post('/update', (req, res) => {
    ...
    res.status(500).json({ success, error: '审批联动更新失败' });
  }
});
...
router.post('/batch-update', (req, res) => {
    ...
    if (!approvals || !Array.isArray(approvals) || approvals.length === 0) {
      return res.status(400).json({ success, error: '缺少审批列表' });
    }

    const results: { id: string; success: boolean; message= [];
    ...
    for (const item of approvals) {
      ...
      if (!businessType || !requestId) {
        results.push({ id, success, message: '业务链接信息不完整' });
        continue;
      }
    ...
    res.status(500).json({ success, error: '批量审批联动更新失败' });
  }
});
...
router.get('/status/:businessType/:requestId', (req, res) => {
    ...
    switch (businessType) {
      ...
      case 'announcement':
        ...
        break;
      default:
        return res.status(400).json({ success, error: `未知的业务类型: ${businessType}` });
    }
    ...
    if (stmt.step()) {
      ...
      stmt.free();
      res.status(404).json({ success, error: '记录不存在' });
    }
  } catch (error) {
    console.error('获取业务表状态失败:', error);
    res.status(500).json({ success, error: '获取业务表状态失败' });
  }
});
