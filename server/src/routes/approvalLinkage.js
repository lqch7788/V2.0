...
import { deductLeaveQuota, deductOvertimeQuota, initEmployeeQuotas, deleteEmployeeQuotas, releaseLeaveQuota } from '../services/leaveQuotaService.js';
const router = Router();
// ============================================
// 辅助函数
// ============================================
 ...
 * 记录操作日志
 */
function logOperation(db, userId, username, action, module, resourceType, resourceId, description) {
 ...
 * 更新物料申请状态
 */
function updateMaterialRequest(db, id, status, approvalCode, extra) {
    try {
        const now = new Date().toISOString();
        db.run(`
 ...
 * 更新采购计划状态
 */
function updatePurchasePlan(db, id, status, approvalCode, extra) {
    try {
        const now = new Date().toISOString();
 ...
 */
function updateProductionPlan(db, id, status, approvalCode, extra) {
    try {
        const now = new Date().toISOString();
        // 映射审批状态到生产计划状态
 ...
 * 更新技术方案状态
 */
function updateTechSolution(db, id, status, approvalCode, extra) {
    try {
        const now = new Date().toISOString();
        // 映射审批状态到技术方案状态
 ...
 * 更新农事任务状态
 */
function updateFarmTask(db, id, status, approvalCode, extra) {
    try {
        const now = new Date().toISOString();
        db.run(`
 ...
 * 更新采收记录状态
 */
function updateHarvestRecord(db, id, status, approvalCode, extra) {
    try {
        const now = new Date().toISOString();
        db.run(`
 ...
 * 更新订单状态
 */
function updateCropOrder(db, id, status, approvalCode, extra) {
    try {
        const now = new Date().toISOString();
        db.run(`
 ...
 * 更新巡查记录状态
 */
function updateInspection(db, id, status, approvalCode, extra) {
    try {
        const now = new Date().toISOString();
        db.run(`
 ...
 * 更新问题记录状态
 */
function updateProblem(db, id, status, approvalCode, extra) {
    try {
        const now = new Date().toISOString();
        db.run(`
      UPDATE problems SET
 ...
 * 更新人工记录状态（请假/加班/调薪等）
 */
function updateLaborRecord(db, id, status, approvalCode, extra) {
    try {
        const now = new Date().toISOString();
        db.run(`
 ...
 * 更新员工状态
 */
function updateEmployee(db, id, status, approvalCode, extra) {
    try {
        const now = new Date().toISOString();
 ...
 * 更新预算状态
 */
function updateBudget(db, id, status, approvalCode, extra) {
    try {
 ...
 * 更新指标状态
 */
function updateIndicator(db, id, status, approvalCode, extra) {
    try {
 ...
 * 更新公告状态
 */
function updateAnnouncement(db, id, status, approvalCode, extra) {
    try {
 ...
 * 根据业务类型更新业务表
 */
export function updateBusinessTable(db, businessType, requestId, action, approvalCode, extra) {
    const now = new Date().toISOString();
    let status = 'pending';
...
router.post('/update', (req, res) => {
    try {
        ...
        // 更新业务表
        const result = updateBusinessTable(db, businessType, requestId, action, approvalCode || '', mergedExtra);
        if (result.success) {
            // 记录操作日志
            logOperation(db, operatorId || 'system', operatorName || '系统', `approval_${action}`, 'approval', businessType, requestId, `审批${action === 'approved' ? '通过' : action === 'rejected' ? '拒绝' : '取消'}，更新业务表: ${result.message}`);
            saveDatabase();
            res.json({
                success: true,
...
router.post('/batch-update', (req, res) => {
    try {
        ...
        for (const item of approvals) {
            ...
            const result = updateBusinessTable(db, businessType, requestId, action, approvalCode || '', extra);
            if (result.success) {
                logOperation(db, operatorId || 'system', operatorName || '系统', `approval_${action}`, 'approval', businessType, requestId, `批量审批${action}，更新业务表: ${result.message}`);
            }
