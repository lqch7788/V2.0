...
router.get('/organizations', (req, res) => {
    ...
    const safeOrder = allowedOrders.includes(order.toLowerCase()) ? order.toLowerCase() : 'asc';
        ...
        const loadChildren = (nodes) => {
            return nodes.map(node => {
                ...
                if (children.length > 0) {
                    node.children = loadChildren(children);
                }
                return node;
            });
        };
    ...
    try {
        ...
        if (inserted && inserted.length > 0) {
            for (const org of inserted) {
                ...
                db.run(`INSERT INTO organizations (id, oid, parent_oid, aid, name, description, address,
        ...
        if (updated && updated.length > 0) {
           ...
           WHERE oid = ?`, [
                ...
                if (orgType === 'department') {
                    ...
                    if (checkStmt.step()) {
                        currentDeptId = (checkStmt.getAsObject()).department_id;
                    }
                    checkStmt.free();
    ...
    try {
        ...
        if (inserted && inserted.length > 0) {
            for (const role of inserted) {
                ...
                db.run(`INSERT INTO roles (id, oid, role_code, role_name, description, status, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
                    id,
                    oid,
        ...
        if (updated && updated.length > 0) {
            for (const role of updated) {
                db.run(`UPDATE roles SET role_code = ?, role_name = ?, description = ?, updated_at = ?
           WHERE oid = ?`, [
                    role.aid || role.oid,
                    role.name,
                    role.description || null,
...
router.post('/auth/login', async (req, res) => {
    ...
    try {
        // 查找用户（支持用户名或真实姓名）
        const stmt = db.prepare('SELECT oid, username, password_hash, real_name, org_oid, org_name, ' +
            ...
            for (const user of inserted) {
                ...
                if (!departmentOid && orgOid) {
                    ...
                    if (orgStmt.step()) {
                        ...
                        departmentOid = orgRow.department_id || null;
                    }
                    orgStmt.free();
                }
                db.run(`INSERT INTO users (id, oid, username, real_name, password_hash, org_oid, department_oid, email, phone, avatar, status, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                    id,
                    oid,
                    username,
            ...
            for (const user of updated) {
                ...
                if (newOrgOid) {
                    ...
                    if (!user.department_oid && !user.departmentOid) {
                        ...
                        if (orgStmt.step()) {
                            const orgRow = orgStmt.getAsObject();
                            const deptId = orgRow.department_id || null;
                            if (deptId) {
                ...
                if (user.department_oid || user.departmentOid) {
                    updates.push('department_oid = ?');
                    bindings.push(user.department_oid || user.departmentOid);
                }
                if (user.username || user.aid) {
                    updates.push('username = ?');
                    bindings.push(user.username || user.aid);
                }
                if (user.real_name || user.realName || user.name) {
                    updates.push('real_name = ?');
                    bindings.push(user.real_name || user.realName || user.name);
                }
                if (user.passwordHash || user.password_hash) {
                    updates.push('password_hash = ?');
                    bindings.push(user.passwordHash || user.password_hash);
                }
                ...
                if (user.phone !== undefined) {
                    updates.push('phone = ?');
                    bindings.push(user.phone);
                }
                if (user.status) {
                    updates.push('status = ?');
                    bindings.push(user.status);
                }
...
router.post('/users/:userOid/roles', authenticate, (req, res) => {
    ...
    try {
        ...
        for (const roleOid of roleOids) {
            db.run('INSERT INTO user_roles (user_oid, role_oid, created_at) VALUES (?, ?, ?)', [userOid, roleOid, new Date().toISOString()]);
        }
        saveDatabase();
    ...
    try {
        ...
        if (inserted && inserted.length > 0) {
            for (const proc of inserted) {
                ...
                db.run(`INSERT INTO processes (oid, parent_oid, process_code, process_name, route, icon,
        ...
        if (updated && updated.length > 0) {
            for (const proc of updated) {
                db.run(`UPDATE processes SET parent_oid = ?, process_code = ?, process_name = ?,
...
router.post('/roles/:roleOid/authority', authenticate, (req, res) => {
    ...
    try {
        ...
        for (const auth of authorities) {
            if (auth.value === -1) {
                // -1 表示删除权限记录
                db.run('DELETE FROM roles_authority WHERE role_oid = ? AND process_oid = ? AND action_oid = ?', [roleOid, auth.processOid, auth.actionOid]);
            }
            else {
                // upsert
                const existing = db.prepare('SELECT id FROM roles_authority WHERE role_oid = ? AND process_oid = ? AND action_oid = ?');
                existing.bind([roleOid, auth.processOid, auth.actionOid]);
                const exists = existing.step();
                existing.free();
                if (exists) {
                    db.run('UPDATE roles_authority SET value = ?, updated_at = ? WHERE role_oid = ? AND process_oid = ? AND action_oid = ?', [auth.value, now, roleOid, auth.processOid, auth.actionOid]);
                }
                else {
                    db.run('INSERT INTO roles_authority (role_oid, process_oid, action_oid, value, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)', [roleOid, auth.processOid, auth.actionOid, auth.value, now, now]);
                }
            }
        }
        saveDatabase();
    ...
    try {
        ...
        if (isAuthorize) {
            for (const orgOid of orgOids) {
                const existing = db.prepare('SELECT id FROM roles_data_authority WHERE role_oid = ? AND org_oid = ?');
                existing.bind([roleOid, orgOid]);
                const exists = existing.step();
                existing.free();
                if (!exists) {
                    db.run('INSERT INTO roles_data_authority (role_oid, org_oid, created_at) VALUES (?, ?, ?)', [roleOid, orgOid, now]);
                }
            }
        }
        else {
...
router.post('/roles/:roleOid/authority/all', authenticate, (req, res) => {
    ...
    try {
        ...
        while (processes.step()) {
            processOids.push(processes.getAsObject().oid);
        }
        processes.free();
        ...
        const actionOids = [];
        while (actions.step()) {
            actionOids.push(actions.getAsObject().oid);
        }
        actions.free();
        // 批量插入权限（忽略冲突）
        for (const processOid of processOids) {
            for (const actionOid of actionOids) {
                try {
                    db.run('INSERT OR IGNORE INTO roles_authority (role_oid, process_oid, action_oid, value, created_at) VALUES (?, ?, ?, 1, ?)', [roleOid, processOid, actionOid, now]);
                }
                catch { /* 忽略冲突 */ }
            }
        }
...
router.post('/roles/:roleOid/authority/process', authenticate, (req, res) => {
    ...
    try {
        ...
        if (value === 0) {
            // 取消权限：删除该工序下所有动作
            for (const actionOid of actionOids) {
                db.run('DELETE FROM roles_authority WHERE role_oid = ? AND process_oid = ? AND action_oid = ?', [roleOid, processOid, actionOid]);
            }
        }
        else {
            // 授予权限
...
router.post('/roles/:roleOid/data-authority/all', authenticate, (req, res) => {
    ...
    try {
        ...
        const orgOids = [];
        while (orgs.step()) {
            orgOids.push(orgs.getAsObject().oid);
        }
        orgs.free();
        for (const orgOid of orgOids) {
            try {
                db.run('INSERT OR IGNORE INTO roles_data_authority (role_oid, org_oid, created_at) VALUES (?, ?, ?)', [roleOid, orgOid, now]);
            }
            catch { /* 忽略冲突 */ }
        }
        saveDatabase();
...
router.post('/users/:userOid/authority', authenticate, (req, res) => {
    ...
    try {
        ...
        for (const auth of authorities) {
            if (auth.value === -1) {
                db.run('DELETE FROM users_authority WHERE user_oid = ? AND process_oid = ? AND action_oid = ?', [userOid, auth.processOid, auth.actionOid]);
            }
            else {
                const existing = db.prepare('SELECT id FROM users_authority WHERE user_oid = ? AND process_oid = ? AND action_oid = ?');
                existing.bind([userOid, auth.processOid, auth.actionOid]);
                const exists = existing.step();
                existing.free();
                if (exists) {
                    db.run('UPDATE users_authority SET value = ?, updated_at = ? WHERE user_oid = ? AND process_oid = ? AND action_oid = ?', [auth.value, now, userOid, auth.processOid, auth.actionOid]);
                }
                else {
                    db.run('INSERT INTO users_authority (user_oid, process_oid, action_oid, value, created_at) VALUES (?, ?, ?, ?, ?)', [userOid, auth.processOid, auth.actionOid, auth.value, now]);
                }
            }
        }
        saveDatabase();
...
router.put('/users/:oid/password', authenticate, async (req, res) => {
    ...
    try {
        ...
        db.run('UPDATE users SET password_hash = ?, updated_at = ? WHERE oid = ?', [newHash, new Date().toISOString(), oid]);
        saveDatabase();
        res.json({ success: true });
    }
    catch (error) {
...
router.put('/users/:oid/status', authenticate, (req, res) => {
    ...
    try {
        db.run('UPDATE users SET status = ?, updated_at = ? WHERE oid = ?', [status, new Date().toISOString(), oid]);
        saveDatabase();
        res.json({ success: true });
    }
    catch (error) {
...
router.get('/my-permissions', authenticate, (req, res) => {
    ...
    `);
        ...
        const isAdmin = user.username === 'admin' ||
            roles.some(r => {
                const code = r.role_code?.toLowerCase();
                return code === 'admin' || code === 'administrators';
            });
        // 获取角色权限汇总（工序-动作）
        const roleOids = roles.map(r => r.oid);
    ...
    `);
      ...
      `);
            ...
            while (dataStmt.step()) {
                dataOrgOids.push(dataStmt.getAsObject().org_oid);
            }
            dataStmt.free();
        }
        res.json({
...
router.get('/check-authority', authenticate, (req, res) => {
    ...
    `);
        ...
        while (userRoleStmt.step()) {
            roleCodes.push(userRoleStmt.getAsObject().role_code);
        }
        userRoleStmt.free();
        ...
        procStmt.bind([processRoute, 'active']);
        const processOid = procStmt.step() ? procStmt.getAsObject().oid : null;
        procStmt.free();
        const actionStmt = db.prepare('SELECT oid FROM actions WHERE action_code = ? AND status = ?');
        actionStmt.bind([actionCode, 'active']);
        const actionOid = actionStmt.step() ? actionStmt.getAsObject().oid : null;
        actionStmt.free();
        ...
        // 检查用户特殊权限（最高优先级）
        const userAuthStmt = db.prepare('SELECT value FROM users_authority WHERE user_oid = ? AND process_oid = ? AND action_oid = ?');
        userAuthStmt.bind([userOid, processOid, actionOid]);
        if (userAuthStmt.step()) {
            const userAuth = userAuthStmt.getAsObject();
            userAuthStmt.free();
    ...
    `);
        ...
        const authorized = roleAuthStmt.step() &&
            (roleAuthStmt.getAsObject().maxValue ?? 0) >= 1;
        roleAuthStmt.free();
