...
router.post('/login', (req, res) => {
  ...
  if (!username || !password) {
    res.status(400).json({ success, error: '请提供账号和密码' });
    return;
  }
  ...
  try {
    ...
    if (stmt.step()) {
      ...
      if (!isPasswordValid) {
        res.status(401).json({ success, error: '账号或密码错误' });
        return;
      }
    ...
    res.status(500).json({ success, error: '登录失败' });
  }
});
...
router.get('/verify', authenticate, (req, res) => {
  if (!req.user) {
    res.status(401).json({ success, error: '未认证' });
    return;
  }
  res.json({
    success,
...
router.post('/auth/login', async (req, res) => {
  ...
  try {
    ...
    const roles= [];
    while (roleStmt.step()) {
      const row = roleStmt.getAsObject(): string };
      ...
      for (const proc of inserted) {
        ...
        db.run(
          ...
          [
            ...
            proc.hidden || proc.is_hidden ? 1,
            'active',
            now,
            now
          ]
        );
        results.inserted.push({ OID, ...proc });
      }
    }
...
router.post('/roles/:roleOid/authority/all', authenticate, (req, res) => {
  ...
  try {
    ...
    saveDatabase();
    res.json({ success, message: `已授予 ${processOids.length} 个工序 × ${actionOids.length} 个动作的全部权限` });
  } catch (error) {
...
router.post('/roles/:roleOid/data-authority/all', authenticate, (req, res) => {
  ...
  try {
    ...
    saveDatabase();
    res.json({ success, message: `已授予 ${orgOids.length} 个组织的数据权限` });
  } catch (error) {
  ...
  try {
    ...
    if (roleOids.length > 0) {
      ...
      const authStmt = db.prepare(`
        SELECT DISTINCT ra.process_oid(${placeholders})
      `);
    ...
    res.json({
      success,
      data: {
        user: { oid, username, realName, orgOid,
        roles: roles.map(r => ({ oid, code, name, isSystem: r.is_system })),
        isAdmin,
        authorities);
  } catch (error) {
...
router.get('/check-authority', authenticate, (req, res) => {
  ...
  try {
    ...
    if (isAdmin) {
      res.json({ success, data: { authorized, isAdmin);
      return;
    }
    ...
    if (!processOid || !actionOid) {
      res.json({ success, data: { authorized, reason: '工序或动作不存在' } });
      return;
    }
    ...
    if (userAuthStmt.step()) {
      const userAuth = userAuthStmt.getAsObject(): number };
      userAuthStmt.free();
      res.json({ success, data: { authorized: userAuth.value === 1, source: 'user_override' } });
      return;
    }
    userAuthStmt.free();
    ...
    const roleAuthStmt = db.prepare(`
      SELECT MAX(ra.value)= ur.role_oid
    ...
    res.json({ success, data, source: authorized ? 'role' : 'none' } });
  } catch (error) {
