...
router.get('/diagnostics', (req, res) => {
    ...
    const tableRows= {};
    if (tables.length > 0) {
      const tableNames = tables[0].values.map(v => v[0]).filter(n => !n.startsWith('sqlite_'));
      for (const name of tableNames) {
        try {
          const count = db.exec(`SELECT COUNT(*) FROM "${name}"`);
          if (count.length > 0 && count[0].values.length > 0) {
            tableRows[name] = count[0].values[0][0];
          }
    ...
    res.json({
      success, data: {
        hmi: { version: 'V2.0.0', buildDate: '2026-05-22', nodeVersion, platform,
        database, dbSize: 'SQLite (运行中)' },
    ...
    res.status(500).json({ success, error: '诊断失败' });
  }
});
...
router.post('/db-test', (req, res) => {
    ...
    res.json({ success, data, status: '连接正常' } });
  } catch (error) {
    res.status(500).json({ success, error: String(error) });
  }
});
...
router.get('/logs', (req, res) => {
    ...
    res.status(500).json({ success, error: '获取调试日志失败' });
  }
});
...
router.delete('/logs', (req, res) => {
    ...
    res.json({ success, message: '调试日志已清空' });
  } catch (error) {
    res.status(500).json({ success, error: '清空日志失败' });
  }
});
