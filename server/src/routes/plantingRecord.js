...
router.get('/', (req, res) => {
    try {
        ...
        const params = [];
        if (facility_oid) {
            conditions.push('pr.facility_oid = ?');
            params.push(facility_oid);
        }
        if (block_oid) {
            conditions.push('pr.block_oid = ?');
            params.push(block_oid);
        }
        if (season_code) {
            conditions.push('pr.season_code = ?');
        ...
        if (year) {
            conditions.push("pr.season_code LIKE ?");
            params.push(`${year}%`);
        }
...
router.post('/', (req, res) => {
    try {
        ...
        const countResult = db.exec("SELECT COUNT(*) as cnt FROM planting_records WHERE facility_oid = ? AND season_code LIKE ? AND deleted_at IS NULL", [facility_oid, `${year}%`]);
    ...
    `, [oid, facility_oid, block_oid || '', seasonCode, crop_variety_oid || '', crop_name, variety_name || '', start_date || now, notes || '', now, now]);
        ...
        db.run('UPDATE greenhouses SET current_crop = ?, current_variety = ?, current_season_code = ?, updated_at = ? WHERE oid = ?', [crop_name, variety_name || '', seasonCode, now, facility_oid]);
    ...
    try {
        ...
        if (status === 'planting') {
            ...
            if (pr.length > 0 && pr[0].values.length > 0) {
                ...
                db.run('UPDATE greenhouses SET current_crop = ?, current_variety = ?, current_season_code = ?, updated_at = ? WHERE oid = ?', [row[1], row[2] || '', row[3], new Date().toISOString(), row[0]]);
            }
        }
