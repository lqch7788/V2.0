...
router.post('/channels', (req, res) => {
    ...
    if (!channelCode || !channelName) {
      return res.status(400).json({ success, error: '渠道编码和名称不能为空' });
    }
    ...
    db.run(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [id, oid, channelCode, channelName, channelType || 'in-app', isActive ? 1);

    res.json({ success, message: '通知渠道创建成功' });
  } catch (error) {
    console.error('创建通知渠道失败:', error);
    res.status(500).json({ success, error: '创建通知渠道失败' });
  }
});
    ...
    if (configJson) {
      db.run(`
        ...
        SET channel_code = COALESCE(?, channel_code),
            ...
            updated_at = ?
        WHERE id = ?
      `, [channelCode, channelName, channelType, isActive !== undefined ? (isActive ? 1) : null, configJson, now, id]);
    } else {
      db.run(`
        UPDATE notification_channels
        SET channel_code = COALESCE(?, channel_code),
            ...
            updated_at = ?
        WHERE id = ?
      `, [channelCode, channelName, channelType, isActive !== undefined ? (isActive ? 1) : null, now, id]);
    }

    res.json({ success, message: '通知渠道更新成功' });
  } catch (error) {
    console.error('更新通知渠道失败:', error);
    res.status(500).json({ success, error: '更新通知渠道失败' });
  }
});
...
router.delete('/channels/:id', (req, res) => {
    ...
    res.json({ success, message: '通知渠道删除成功' });
  } catch (error) {
    console.error('删除通知渠道失败:', error);
    res.status(500).json({ success, error: '删除通知渠道失败' });
  }
});
...
router.patch('/channels/:id/toggle', (req, res) => {
    ...
    if (result.length === 0 || result[0].values.length === 0) {
      return res.status(404).json({ success, error: '通知渠道不存在' });
    }
    ...
    res.json({ success, message: '通知渠道状态切换成功' });
  } catch (error) {
    console.error('切换通知渠道状态失败:', error);
    res.status(500).json({ success, error: '切换通知渠道状态失败' });
  }
});
...
router.post('/rules', (req, res) => {
    ...
    if (!ruleCode || !ruleName) {
      return res.status(400).json({ success, error: '规则编码和名称不能为空' });
    }
    ...
    db.run(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [id, oid, ruleCode, ruleName, eventType || '', recipientType || '', recipientIdsJson || '[]', channelIdsJson || '[]', frequency || 'immediate', template || '', isActive ? 1);

    res.json({ success, message: '通知规则创建成功' });
  } catch (error) {
    console.error('创建通知规则失败:', error);
    res.status(500).json({ success, error: '创建通知规则失败' });
  }
});
...
router.put('/rules/:id', (req, res) => {
    ...
    db.run(`
      ...
      SET rule_code = COALESCE(?, rule_code),
          ...
          updated_at = ?
      WHERE id = ?
    `, [ruleCode, ruleName, eventType, recipientType, recipientIdsJson, channelIdsJson, frequency, template, isActive !== undefined ? (isActive ? 1) : null, now, id]);

    res.json({ success, message: '通知规则更新成功' });
  } catch (error) {
    console.error('更新通知规则失败:', error);
    res.status(500).json({ success, error: '更新通知规则失败' });
  }
});
...
router.delete('/rules/:id', (req, res) => {
    ...
    res.json({ success, message: '通知规则删除成功' });
  } catch (error) {
    console.error('删除通知规则失败:', error);
    res.status(500).json({ success, error: '删除通知规则失败' });
  }
});
...
router.patch('/rules/:id/toggle', (req, res) => {
    ...
    if (result.length === 0 || result[0].values.length === 0) {
      return res.status(404).json({ success, error: '通知规则不存在' });
    }
    ...
    res.json({ success, message: '通知规则状态切换成功' });
  } catch (error) {
    console.error('切换通知规则状态失败:', error);
    res.status(500).json({ success, error: '切换通知规则状态失败' });
  }
});
...
router.get('/preferences/:userOid', (req, res) => {
    ...
    res.status(500).json({ success, error: '获取通知偏好失败' });
  }
});
...
router.put('/preferences/:userOid', (req, res) => {
    ...
    if (existing.length > 0 && existing[0].values.length > 0) {
      ...
      db.run(`
        ...
        WHERE user_oid = ?
      `, [
        approvalNotify !== undefined ? (approvalNotify ? 1) : null,
        alertNotify !== undefined ? (alertNotify ? 1) : null,
        dailySummary !== undefined ? (dailySummary ? 1) : null,
        announcementNotify !== undefined ? (announcementNotify ? 1) : null,
        dndEnabled !== undefined ? (dndEnabled ? 1) : null,
      ...
      `, [
        userOid,
        approvalNotify !== false ? 1,
        alertNotify !== false ? 1,
        dailySummary ? 1,
        announcementNotify !== false ? 1,
        dndEnabled ? 1,
        dndStartTime || '22:00',
        dndEndTime || '08:00',
        now, now,
    ...
    res.json({ success, message: '通知偏好保存成功' });
  } catch (error) {
    console.error('保存通知偏好失败:', error);
    res.status(500).json({ success, error: '保存通知偏好失败' });
  }
});
