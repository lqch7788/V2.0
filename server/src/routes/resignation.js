 ...
 * GET /api/resignation
 */
router.get('/', (req, res) => { worker_name, resignation_type, status, start_date, end_date, page = 1, limit = 50 } = req.query;
    ...
    res.status(500).json({ success, error: '获取离职记录失败' });
  }
});
...
router.get('/:id', (req, res) => { id } = req.params;
    ...
    if (!item || Object.keys(item).length === 0) {
      return res.status(404).json({ success, error: '离职记录不存在' });
    }
    ...
    res.status(500).json({ success, error: '获取离职记录详情失败' });
  }
});
...
router.post('/', (req, res) => {
      ...
      handover_user_name, handover_note, remarks
    } = req.body;
    ...
    saveDatabase();
    res.status(201).json({ success, data: { id, resignation_code);
  } catch (error) {
    console.error('创建离职记录失败:', error);
    res.status(500).json({ success, error: '创建离职记录失败' });
  }
});
...
router.put('/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '更新离职记录失败' });
  }
});
...
router.delete('/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '删除离职记录失败' });
  }
});
 ...
 */
router.post('/:id/leave', (req, res) => { id } = req.params;
    ...
    saveDatabase();
    res.json({ success, message: '离职操作成功' });
  } catch (error) {
    console.error('离职操作失败:', error);
    res.status(500).json({ success, error: '离职操作失败' });
  }
});
