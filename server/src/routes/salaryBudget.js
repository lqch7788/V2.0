 ...
 */
router.get('/', (req, res) => { dept_id, budget_month, status, page = 1, limit = 50 } = req.query;
    ...
    res.status(500).json({ success, error: '获取工资预算记录失败' });
  }
});
...
router.get('/:id', (req, res) => { id } = req.params;
    ...
    if (!item || Object.keys(item).length === 0) {
      return res.status(404).json({ success, error: '工资预算记录不存在' });
    }
    ...
    res.status(500).json({ success, error: '获取工资预算记录详情失败' });
  }
});
...
router.post('/', (req, res) => {
      ...
      total_base_salary, total_overtime_pay, total_bonus, remark,
    ...
    saveDatabase();
    res.status(201).json({ success, data: { id, budget_code);
  } catch (error) {
    console.error('创建工资预算记录失败:', error);
    res.status(500).json({ success, error: '创建工资预算记录失败' });
  }
});
...
router.put('/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '更新工资预算记录失败' });
  }
});
...
router.delete('/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '删除工资预算记录失败' });
  }
});
