...
import { updateBusinessTable } from './approvalLinkage.js';
const router = Router();
// ============================================
// 辅助函数
// ============================================
...
function canApproveApproval(approval, userId, userRoles) {
    ...
    const currentApprover = approval.approvers.find((a) => a.order === approval.currentStep && a.status === 'pending');
    if (!currentApprover) {
        return false;
    }
    // 检查是否是当前审批人
...
router.get('/:id', (req, res) => {
    try {
        ...
        result.nextApprover = result.approvers.find((a) => a.order === currentStep && a.status === 'pending') || null;
        res.json({ success: true, data: result });
    }
    catch (error) {
...
router.post('/', (req, res) => {
    try {
        const db = getDatabase();
        const { id, code, type, typeName, category, title, description, applicantId, applicantName, applicantDepartment, applyDate, applyTime, currentStep, totalSteps, approvers, records, status, businessLink, attachments, priority, dueDate, relatedBatchCode, relatedTaskIds, amount, materials, workflowId, workflowName, } = req.body;
        if (!id || !type || !title) {
    ...
    `, [
        ...
        if (newStatus === 'approved' || newStatus === 'rejected' || newStatus === 'cancelled' || newStatus === 'partially_approved') {
            ...
            if (businessLink?.type && businessLink?.requestId) {
                try {
                    const linkageAction = newStatus === 'approved' ? 'approved'
                        : newStatus === 'rejected' ? 'rejected'
                            : newStatus === 'cancelled' ? 'cancelled'
                                : 'partially_approved';
                    const result = updateBusinessTable(db, businessLink.type, businessLink.requestId, linkageAction, approval.code, businessLink);
                    if (result.success) {
        ...
        if (updatedApproval) {
            ['approvers', 'records', 'business_link', 'attachments', 'materials', 'related_task_ids'].forEach(field => {
                if (typeof updatedApproval[field] === 'string') {
                    try {
                        updatedApproval[field] = JSON.parse(updatedApproval[field]);
                    }
                    catch { /* keep original */ }
                }
            });
...
router.get('/submitted/me', (req, res) => {
    try {
        ...
        const stmt = db.prepare("SELECT * FROM approvals WHERE applicant_id = ? ORDER BY created_at DESC");
        stmt.bind([userId]);
        const approvals = [];
        while (stmt.step()) {
            approvals.push(stmt.getAsObject());
    ...
    try {
        ...
        for (const id of approvalIds) {
            try {
                ...
                let newCurrentStep = approval.current_step;
                // 查找当前步骤的审批人
                const currentApproverIndex = approvers.findIndex((a) => a.order === newCurrentStep && a.status === 'pending');
