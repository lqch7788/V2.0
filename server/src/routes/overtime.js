...
// 英文状态值到中文的映射
const OVERTIME_STATUS_LABEL_MAP= {
  'pending': '待审批',
  'approved': '已通过',
  'rejected': '已拒绝',
  'cancelled': '已取消',
 ...
 * 标准化加班类型值（将中文转换为英文）
 */
function normalizeOvertimeType(overtimeType?: string){
 ...
 * 标准化加班状态值（将中文转换为英文）
 */
function normalizeOvertimeStatus(status?: string){
 ...
 * 获取加班类型显示标签
 */
function getOvertimeTypeLabel(overtimeType){
 ...
 * @returns 加班费
 */
function calculateOvertimePay(baseSalary, hours, overtimeType){
...
router.get('/', (req, res) => { worker_name, overtime_type, status, start_date, end_date, page = 1, limit = 50 } = req.query;
    ...
    if (overtime_type) {
      ...
      params.push(normalizeOvertimeType(overtime_type));
    }

    if (status) {
      sql += ' AND status = ?';
      params.push(normalizeOvertimeStatus(status));
    }
    ...
    res.status(500).json({ success, error: '获取加班记录失败' });
  }
});
...
router.get('/:id', (req, res) => { id } = req.params;
    ...
    if (!item || Object.keys(item).length === 0) {
      return res.status(404).json({ success, error: '加班记录不存在' });
    }
    ...
    res.status(500).json({ success, error: '获取加班记录详情失败' });
  }
});
...
router.post('/', (req, res) => {
    ...
    res.status(500).json({ success, error: '创建加班记录失败' });
  }
});
...
router.put('/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '更新加班记录失败' });
  }
});
...
router.delete('/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '删除加班记录失败' });
  }
});
