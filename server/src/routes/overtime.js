 ...
 * 标准化加班类型值（将中文转换为英文）
 */
function normalizeOvertimeType(overtimeType) {
    if (!overtimeType)
        return 'workday';
 ...
 * 标准化加班状态值（将中文转换为英文）
 */
function normalizeOvertimeStatus(status) {
    if (!status)
        return 'pending';
...
router.post('/', (req, res) => {
    try {
        const { id, worker_id, worker_name, overtime_type, work_date, start_time, end_time, hours, base_salary, reason, status, department_id, department_name, greenhouse_id, greenhouse_name, remarks } = req.body;
