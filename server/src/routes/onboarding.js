 ...
 * 统一响应格式
 */
function successResponse(res, data, message?: string) {
...
router.post('/', (req, res) => {
    ...
    const progress = JSON.stringify([
      { step, name: '资料提交', status: 'completed', completedAt: now.split('T')[0] },
      { step, name: '合同签订', status: 'pending' },
      { step, name: '入职培训', status: 'pending' },
      { step, name: '档案创建', status: 'pending' },
    ]);
...
router.put('/:id', (req, res) => {
    ...
    db.run(
      ...
      [
        ...
        operatorId, operatorName, status === '已入职' ? now);
...
router.post('/batch-delete', (req, res) => {
    ...
    successResponse(res, { deletedCount, '批量删除成功');
  } catch (error) {
