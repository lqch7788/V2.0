...
router.get('/marks/all', (req, res) => {
    try {
        ...
        const items = queryToObjects(db, `SELECT * FROM plant_marks WHERE is_use = 1 ORDER BY sort_order, id`);
...
router.post('/marks/assign', (req, res) => {
    try {
        ...
        if (mark.length === 0) {
            res.status(404).json({ success: false, error: '标记不存在' });
            return;
        }
        ...
        VALUES (?, 'mark', ?, ?, ?, ?)`, [labelId, mark_id, mark[0].name, mark[0].color, now]);
            count++;
        }
...
router.post('/marks', (req, res) => {
    try {
        ...
        if (!name) {
            res.status(400).json({ success: false, error: 'name 为必填项' });
            return;
        }
        db.run(`INSERT INTO plant_marks (name, color, icon, parent_id, mark_aid, sort_order) VALUES (?,?,?,?,?,?)`, [name, color || null, icon || null, parent_id || 0, mark_aid || '', sort_order || 0]);
...
router.put('/marks/:id', (req, res) => {
    try {
        ...
        db.run(`UPDATE plant_marks SET name=?, color=?, icon=?, parent_id=?, mark_aid=?, sort_order=?, is_use=? WHERE id=?`, [name ?? exist[0].name, color ?? exist[0].color, icon ?? exist[0].icon,
...
router.post('/generate-batch', (req, res) => {
    try {
        ...
        for (let i = 0; i < count; i++) {
            ...
            db.run(`INSERT INTO plant_labels (label_number, planting_id, seedling_id, move_in_area_name, move_in_date, create_time)
         VALUES (?, ?, ?, ?, ?, ?)`, [labelNumber, planting_id || null, seedling_id || null, area_name || null, start_date || null, now]);
            labels.push({
                labelNumber,
...
router.get('/', (req, res) => {
    try {
        ...
        const params = [];
        if (planting_id) {
            conditions.push('planting_id = ?');
            params.push(planting_id);
        }
        const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
        const total = db.exec(`SELECT COUNT(*) as cnt FROM plant_labels ${whereClause}`, params)[0]?.values[0]?.[0] ?? 0;
        const offset = (pageNum - 1) * limitNum;
        const items = queryToObjects(db, `SELECT * FROM plant_labels ${whereClause} ORDER BY id DESC LIMIT ? OFFSET ?`, [...params, limitNum, offset]);
...
router.get('/query-by-label', (req, res) => {
    try {
        ...
        const params = [];
        if (label_code) {
            conditions.push('label_code LIKE ?');
...
router.get('/:id/resumes', (req, res) => {
    try {
        const db = getDatabase();
        const items = queryToObjects(db, `SELECT * FROM plant_label_resume WHERE label_id = ? ORDER BY operation_date DESC`, [req.params.id]);
        res.json({ success: true, data: items });
    }
    catch (error) {
        res.status(500).json({ success: false, error: error.message });
...
router.post('/:id/resumes', (req, res) => {
    try {
        ...
        if (operation_type === 'move_in') {
            db.run(`UPDATE plant_labels SET move_in_area_name = ?, move_in_date = ? WHERE id = ?`, [to_area_name || '', operation_date, labelId]);
        }
        else if (operation_type === 'move_out') {
            db.run(`UPDATE plant_labels SET move_out_area_name = ?, move_out_date = ? WHERE id = ?`, [to_area_name || '', operation_date, labelId]);
        }
      ...
      VALUES (?,?,?,?,?,?,?,?,?)`, [labelId, operation_type, from_area_name || null, to_area_name || null, mark_id || null,
