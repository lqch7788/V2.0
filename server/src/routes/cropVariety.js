 ...
 * 获取所有扩展类别
 */
router.get('/extensions/categories', (req, res) => {
    const db = getDatabase();
    const results = db.exec('SELECT * FROM crop_variety_category_extensions ORDER BY sort_order, category_code');

    if (results.length === 0) {
      return res.json({ success: true, data: varieties);
    }

    const { columns, values } = results[0];
    const extensions = values.map((row) => {
      const obj= {};
    ...
    res.status(500).json({ success, error: '获取类别扩展失败' });
  }
});
 ...
 * 创建类别扩展
 */
router.post('/extensions/categories', (req, res) => { id, category_code, category_name, sort_order = 0, status = 'active' } = req.body;
    ...
    res.status(500).json({ success, error: '创建类别扩展失败' });
  }
});
 ...
 * 删除类别扩展
 */
router.delete('/extensions/categories/:id', (req, res) => { id } = req.params;
    const db = getDatabase();
    ...
    res.status(500).json({ success, error: '删除类别扩展失败' });
  }
});
 ...
 * 更新类别扩展
 */
router.put('/extensions/categories/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '更新类别扩展失败' });
  }
});
 ...
 * 获取指定类别的所有扩展类型
 */
router.get('/extensions/types/:categoryCode', (req, res) => { categoryCode } = req.params;
    const db = getDatabase();
    ...
    res.status(500).json({ success, error: '获取类型扩展失败' });
  }
});
 ...
 * 获取所有扩展类型
 */
router.get('/extensions/types', (req, res) => {
    const db = getDatabase();
 ...
 * 创建类型扩展
 */
router.post('/extensions/types', (req, res) => { id, category_code, category_name, type_code, type_name, sort_order = 0, status = 'active' } = req.body;
    ...
    res.status(500).json({ success, error: '创建类型扩展失败' });
  }
});
 ...
 * 删除类型扩展
 */
router.delete('/extensions/types/:id', (req, res) => { id } = req.params;
    const db = getDatabase();
    ...
    res.status(500).json({ success, error: '删除类型扩展失败' });
  }
});
 ...
 * 更新类型扩展
 */
router.put('/extensions/types/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '更新类型扩展失败' });
  }
});
 ...
 */
router.get('/extensions/varieties', (req, res) => {
    const db = getDatabase();
    ...
    res.status(500).json({ success, error: '获取品种扩展失败' });
  }
});
 ...
 * 获取指定类型的所有扩展品种
 */
router.get('/extensions/varieties/:categoryCode/:typeCode', (req, res) => { categoryCode, typeCode } = req.params;
    const db = getDatabase();
 ...
 * 创建品种扩展
 */
router.post('/extensions/varieties', (req, res) => { id, category_code, type_code, variety_code, variety_name, sort_order = 0, status = 'active' } = req.body;
    ...
    res.status(500).json({ success, error: '创建品种扩展失败' });
  }
});
 ...
 * 删除品种扩展
 */
router.delete('/extensions/varieties/:id', (req, res) => { id } = req.params;
    const db = getDatabase();
    ...
    res.status(500).json({ success, error: '删除品种扩展失败' });
  }
});
 ...
 * 更新品种扩展
 */
router.put('/extensions/varieties/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '更新品种扩展失败' });
  }
});
 ...
 */
router.get('/extensions/subvariety1', (req, res) => {
    const db = getDatabase();
    ...
    res.status(500).json({ success, error: '获取子品种1扩展失败' });
  }
});
 ...
 * 获取指定品种的所有扩展子品种1
 */
router.get('/extensions/subvariety1/:categoryCode/:typeCode/:varietyCode', (req, res) => { categoryCode, typeCode, varietyCode } = req.params;
    const db = getDatabase();
    const results = db.exec(
 ...
 * 创建子品种1扩展
 */
router.post('/extensions/subvariety1', (req, res) => { id, category_code, type_code, variety_code, sub_variety1_code, sub_variety1_name, sort_order = 0, status = 'active' } = req.body;
    ...
    res.status(500).json({ success, error: '创建子品种1扩展失败' });
  }
});
 ...
 * 删除子品种1扩展
 */
router.delete('/extensions/subvariety1/:id', (req, res) => { id } = req.params;
    const db = getDatabase();
    ...
    res.status(500).json({ success, error: '删除子品种1扩展失败' });
  }
});
 ...
 * 更新子品种1扩展
 */
router.put('/extensions/subvariety1/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '更新子品种1扩展失败' });
  }
});
 ...
 */
router.post('/bulk', (req, res) => { varieties } = req.body;
    if (!Array.isArray(varieties) || varieties.length === 0) {
      return res.status(400).json({ success, error: 'varieties 数组不能为空' });
    }
    ...
    while (existingStmt.step()) {
      ...
      if (row.crop_code) existingCodes.add(row.crop_code);
    }
    existingStmt.free();
    ...
    res.json({
      success,
      data, total: varieties.length }
    });
  } catch (error) {
    console.error('批量导入作物品种失败:', error);
    res.status(500).json({ success, error: '批量导入作物品种失败' });
  }
});
