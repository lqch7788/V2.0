...
router.post('/', (req, res) => {
    try {
        ...
        values.push(now, now);
        const db = getDatabase();
        db.run(`INSERT INTO crop_varieties (${columns.join(', ')}) VALUES (${placeholders.join(', ')})`, values);
        saveDatabase();
        // 返回完整记录
        const stmt = db.prepare('SELECT * FROM crop_varieties WHERE id = ?');
        stmt.bind([newId]);
 ...
 * 创建类别扩展
 */
router.post('/extensions/categories', (req, res) => {
    try {
 ...
 * 删除类别扩展
 */
router.delete('/extensions/categories/:id', (req, res) => {
    try {
        const { id } = req.params;
        const db = getDatabase();
 ...
 * 更新类别扩展
 */
router.put('/extensions/categories/:id', (req, res) => {
    try {
        const { id } = req.params;
        const { category_name, sort_order, status } = req.body;
        const db = getDatabase();
        const now = new Date().toISOString();
        db.run(`UPDATE crop_variety_category_extensions SET category_name = ?, sort_order = ?, status = ?, updated_at = ? WHERE id = ?`, [category_name, sort_order || 0, status || 'active', now, id]);
        saveDatabase();
        res.json({ success: true, data: { id } });
    }
    catch (error) {
 ...
 * 获取指定类别的所有扩展类型
 */
router.get('/extensions/types/:categoryCode', (req, res) => {
    try {
 ...
 * 创建类型扩展
 */
router.post('/extensions/types', (req, res) => {
    try {
 ...
 * 删除类型扩展
 */
router.delete('/extensions/types/:id', (req, res) => {
    try {
        const { id } = req.params;
        const db = getDatabase();
 ...
 * 更新类型扩展
 */
router.put('/extensions/types/:id', (req, res) => {
    try {
        const { id } = req.params;
        const { type_name, sort_order, status } = req.body;
        const db = getDatabase();
        const now = new Date().toISOString();
        db.run(`UPDATE crop_variety_type_extensions SET type_name = ?, sort_order = ?, status = ?, updated_at = ? WHERE id = ?`, [type_name, sort_order || 0, status || 'active', now, id]);
        saveDatabase();
        res.json({ success: true, data: { id } });
    }
    catch (error) {
 ...
 * 获取指定类型的所有扩展品种
 */
router.get('/extensions/varieties/:categoryCode/:typeCode', (req, res) => {
    try {
        const { categoryCode, typeCode } = req.params;
        const db = getDatabase();
        const results = db.exec(`SELECT * FROM crop_variety_variety_extensions WHERE category_code = ? AND type_code = ? ORDER BY sort_order, variety_code`, [categoryCode, typeCode]);
        if (results.length === 0) {
            return res.json({ success: true, data: [] });
        }
        const { columns, values } = results[0];
 ...
 * 创建品种扩展
 */
router.post('/extensions/varieties', (req, res) => {
    try {
 ...
 * 删除品种扩展
 */
router.delete('/extensions/varieties/:id', (req, res) => {
    try {
        const { id } = req.params;
        const db = getDatabase();
 ...
 * 更新品种扩展
 */
router.put('/extensions/varieties/:id', (req, res) => {
    try {
        const { id } = req.params;
        const { variety_name, sort_order, status } = req.body;
        const db = getDatabase();
        const now = new Date().toISOString();
        db.run(`UPDATE crop_variety_variety_extensions SET variety_name = ?, sort_order = ?, status = ?, updated_at = ? WHERE id = ?`, [variety_name, sort_order || 0, status || 'active', now, id]);
        saveDatabase();
        res.json({ success: true, data: { id } });
    }
    catch (error) {
 ...
 * 获取指定品种的所有扩展子品种1
 */
router.get('/extensions/subvariety1/:categoryCode/:typeCode/:varietyCode', (req, res) => {
    try {
        const { categoryCode, typeCode, varietyCode } = req.params;
        const db = getDatabase();
        const results = db.exec(`SELECT * FROM crop_variety_sub1_extensions WHERE category_code = ? AND type_code = ? AND variety_code = ? ORDER BY sort_order, sub_variety1_code`, [categoryCode, typeCode, varietyCode]);
        if (results.length === 0) {
            return res.json({ success: true, data: [] });
        }
        const { columns, values } = results[0];
 ...
 * 创建子品种1扩展
 */
router.post('/extensions/subvariety1', (req, res) => {
    try {
 ...
 * 删除子品种1扩展
 */
router.delete('/extensions/subvariety1/:id', (req, res) => {
    try {
        const { id } = req.params;
        const db = getDatabase();
 ...
 * 更新子品种1扩展
 */
router.put('/extensions/subvariety1/:id', (req, res) => {
    try {
        const { id } = req.params;
        const { sub_variety1_name, sort_order, status } = req.body;
        const db = getDatabase();
        const now = new Date().toISOString();
        db.run(`UPDATE crop_variety_sub1_extensions SET sub_variety1_name = ?, sort_order = ?, status = ?, updated_at = ? WHERE id = ?`, [sub_variety1_name, sort_order || 0, status || 'active', now, id]);
        saveDatabase();
        res.json({ success: true, data: { id } });
    }
    catch (error) {
...
router.post('/bulk', (req, res) => {
    try {
        ...
        while (existingStmt.step()) {
            ...
            if (row.crop_code)
                existingCodes.add(row.crop_code);
        }
        existingStmt.free();
        // 所有支持的字段
        const fields = [
