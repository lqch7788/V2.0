...
router.post('/', (req, res) => {
    ...
    if (!body.fertilizer_name) {
      res.status(400).json({ success, error: '肥料名称为必填项' });
      return;
    }
    const code = generateFertilizerCode(db);
...
router.get('/:id', (req, res) => {
    ...
    if (fertilizer.length === 0) { res.status(404).json({ success, error: '肥料不存在' }); return; }
...
router.put('/:id', (req, res) => {
    ...
    if (existing.length === 0) { res.status(404).json({ success, error: '肥料不存在' }); return; }
