...
function generateFertilizerCode(db) {
    ...
    const allCodes = queryToObjects(db, `SELECT fertilizer_code FROM fertilizer_library`);
    let maxSeq = 0;
...
router.get('/', (req, res) => {
    try {
        ...
        const { fertilizer_type, keyword, page = '1', limit = '10000' } = req.query;
        ...
        const items = queryToObjects(db, `SELECT * FROM fertilizer_library ${whereClause} ORDER BY create_time DESC LIMIT ? OFFSET ?`, [...params, limitNum, offset]);
        // 为每个肥料查询对应的规格数据
        for (const item of items) {
            const specs = queryToObjects(db, `SELECT * FROM fertilizer_specs WHERE fertilizer_id = ? ORDER BY create_time DESC`, [item.id]);
            item.specs = specs;
        }
...
router.post('/', (req, res) => {
    ...
    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`, [id, code, body.fertilizer_name, body.fertilizer_type || null, body.application_timing || null,
...
router.get('/:id', (req, res) => {
    try {
        ...
        if (fertilizer.length === 0) {
            res.status(404).json({ success: false, error: '肥料不存在' });
            return;
        }
...
router.put('/:id', (req, res) => {
    try {
        ...
        const { id } = req.params;
        const body = req.body;
        const existing = queryToObjects(db, `SELECT * FROM fertilizer_library WHERE id = ?`, [id]);
      ...
      function_desc=?, taboo_desc=?, shelf_life=?, storage_condition=?, supplier_info=?,
      status=?, update_time=? WHERE id=?`, [body.fertilizer_name ?? existing[0].fertilizer_name,
...
router.post('/:id/specs', (req, res) => {
    ...
    ) VALUES (?,?,?,?,?,?,?,?,?,?,?)`, [specId, id, body.brand_name || null, body.spec_content || null, body.manufacturer || null,
...
router.put('/specs/:specId', (req, res) => {
    try {
        ...
        const body = req.body;
        const existing = queryToObjects(db, `SELECT * FROM fertilizer_specs WHERE id = ?`, [specId]);
        if (existing.length === 0) {
            res.status(404).json({ success: false, error: '规格不存在' });
            return;
        }
        db.run(`UPDATE fertilizer_specs SET brand_name=?, spec_content=?, manufacturer=?,
      suggested_dosage=?, suggested_ratio=?, dosage_unit=?, remark=?, status=? WHERE id=?`, [body.brand_name ?? existing[0].brand_name,
