    ...
    try {
        ...
        if (inserted && inserted.length > 0) {
            for (const config of inserted) {
                ...
                db.run(`INSERT INTO system_configs (id, config_key, config_value, config_type, category, description, is_active, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)`, [
                    id,
        ...
        if (updated && updated.length > 0) {
            for (const config of updated) {
                db.run(`UPDATE system_configs SET config_key = ?, config_value = ?, config_type = ?, category = ?, description = ?, updated_at = ?
           WHERE id = ?`, [
                    config.config_key || config.configKey,
                    config.config_value || config.configValue || '',
                    config.config_type || config.configType || 'string',
