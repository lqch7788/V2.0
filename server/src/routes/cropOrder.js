...
router.get('/', (req, res) => {
    try {
        ...
        const { crop_name, status, order_type, keyword, page = 1, limit = 50 } = req.query;
...
router.get('/:id', (req, res) => {
    try {
        ...
        const items = queryToObjects(db, 'SELECT * FROM crop_orders WHERE id = ?', [id]);
...
router.post('/', (req, res) => {
    try {
        const db = getDatabase();
        let { id, order_code, order_type, crop_name, crop_variety, quantity, unit, unit_price, total_amount, customer_id, customer_name, customer_phone, delivery_address, order_date, expected_delivery_date, actual_delivery_date, expected_completion_date, status, remarks, create_by, 
        // 新增字段
        order_name, crop_category, planned_quantity, completed_quantity, actual_quantity, expected_harvest_date, supplier_name, instance_ids } = req.body;
        // 如果没有提供id，则自动生成一个
        if (!id) {
            id = 'ORD' + Date.now();
        }
    ...
    `, [
        ...
        const newOrders = queryToObjects(db, 'SELECT * FROM crop_orders WHERE id = ?', [id]);
...
router.put('/:id', (req, res) => {
    try {
        ...
        const updatedOrders = queryToObjects(db, 'SELECT * FROM crop_orders WHERE id = ?', [id]);
        res.json({
            success: true,
            message: '订单更新成功',
 ...
 */
router.post('/:id/link-instances', (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.post('/:id/unlink-instances', (req, res) => {
    try {
        const { id } = req.params;
        const { instanceIds } = req.body;
        if (!Array.isArray(instanceIds) || instanceIds.length === 0) {
