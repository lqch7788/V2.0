...
router.get('/', (req, res) => {
    ...
    res.status(500).json({ success, error: '获取订单列表失败' });
  }
});
...
router.get('/:id', (req, res) => { id } = req.params;
    ...
    if (!items || items.length === 0) {
      return res.status(404).json({ success, error: '订单不存在' });
    }

    res.json({ success: true, data: items);
  } catch (error) {
    console.error('获取订单详情失败:', error);
    res.status(500).json({ success, error: '获取订单详情失败' });
  }
});
...
router.post('/', (req, res) => {
    ...
    if (!code || !code.startsWith('DD')) {
      ...
      try {
        ...
        let maxSeq = 0;
        if (stmt.step()) {
          const lastCode = stmt.getAsObject().order_code;
    ...
    res.status(500).json({ success, error: '创建订单失败' });
  }
});
...
router.put('/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '更新订单失败' });
  }
});
...
router.delete('/:id', (req, res) => { id } = req.params;
    ...
    res.json({ success, message: '订单删除成功' });
  } catch (error) {
    console.error('删除订单失败:', error);
    res.status(500).json({ success, error: '删除订单失败' });
  }
});
 ...
 * Body: { ids: string[] }
 */
router.post('/batch/delete', (req, res) => { ids } = req.body;

    if (!Array.isArray(ids) || ids.length === 0) {
      return res.status(400).json({ success, error: '请提供要删除的订单ID列表' });
    }

    const db = getDatabase();
    const deletedIds= [];
    const failedIds: { id: string; reason= [];
    ...
    res.status(500).json({ success, error: '批量删除订单失败' });
  }
});
...
router.get('/stats/summary', (req, res) => {
    ...
    const sql = `
      SELECT
        COUNT(*)(CASE WHEN status = 'planned' THEN 1 ELSE 0 END)(CASE WHEN status = 'in_progress' THEN 1 ELSE 0 END)(CASE WHEN status = 'completed' THEN 1 ELSE 0 END)(CASE WHEN status = 'cancelled' THEN 1 ELSE 0 END)(total_amount)`;
    ...
    res.status(500).json({ success, error: '获取订单统计失败' });
  }
});
 ...
 * Query, end_date
 */
router.get('/stats/by-date-range', (req, res) => {
    const db = getDatabase();
    const { start_date, end_date } = req.query;

    if (!start_date || !end_date) {
      return res.status(400).json({ success, error: '请提供开始日期和结束日期' });
    }

    const sql = `
      SELECT
        DATE(order_date)(*)(total_amount)= ? AND order_date <= ?
      GROUP BY DATE(order_date)
      ORDER BY date DESC
    `;

    const stmt = db.prepare(sql);
    stmt.bind([start_date);
    ...
    res.status(500).json({ success, error: '获取日期范围统计失败' });
  }
});
...
router.get('/export', (req, res) => {
    ...
    if (format === 'json') {
      ...
      res.json({
        success,
        data,
        meta: { total, exported_at: new Date().toISOString() }
      });
    } else {
      // 返回 CSV 格式
      if (items.length === 0) {
        return res.status(404).json({ success, error: '没有可导出的数据' });
      }
    ...
    res.status(500).json({ success, error: '导出订单失败' });
  }
});
...
router.get('/export/stats', (req, res) => {
    ...
    res.status(500).json({ success, error: '导出订单统计失败' });
  }
});
 ...
 */
router.post('/:id/link-instances', (req, res) => { id } = req.params;
    const { instanceIds } = req.body;

    if (!Array.isArray(instanceIds) || instanceIds.length === 0) {
      return res.status(400).json({ success, error: '缺少 instanceIds 参数或 instanceIds 不是有效数组' });
    }
    ...
    for (const instanceId of instanceIds) {
      ...
      if (checkStmt.step()) {
        currentOrderId = checkStmt.getAsObject().order_id;
      }
      checkStmt.free();
    ...
    saveDatabase();
    res.json({ success, data: { linked);
  } catch (error) {
    console.error('关联实例失败:', error);
    res.status(500).json({ success, error: '关联实例失败' });
  }
});
 ...
 */
router.post('/:id/unlink-instances', (req, res) => { id } = req.params;
    const { instanceIds } = req.body;
    ...
    saveDatabase();
    res.json({ success, data: { unlinked);
  } catch (error) {
    console.error('取消关联实例失败:', error);
    res.status(500).json({ success, error: '取消关联实例失败' });
  }
});
...
router.put('/:id/status', (req, res) => { id } = req.params;
    ...
    res.json({ success, message: '订单状态已更新' });
  } catch (error) {
    console.error('更新订单状态失败:', error);
    res.status(500).json({ success, error: '更新订单状态失败' });
  }
});
...
router.post('/reset', (req, res) => {
    ...
    saveDatabase();
    res.json({ success, message: '订单数据已重置' });
  } catch (error) {
    console.error('重置订单数据失败:', error);
    res.status(500).json({ success, error: '重置订单数据失败' });
  }
});
