...
router.post('/', (req, res) => {
    try {
        ...
        const { id, task_code, taskCode, task_title, taskTitle, title, task_type, taskType, type, task_content, taskContent, assignee_id, assigneeId, assignee_name, assigneeName, assigner_id, assignerId, assigner_name, assignerName, greenhouse_id, greenhouseId, greenhouse_name, greenhouseName, area_name, areaName, plan_date, planDate, plan_time, planTime, priority, status, create_by, createBy, due_date, dueDate, progress, crop, cropName, estimated_hours, estimatedHours, estimated_days, estimatedDays, remarks, materials, tools, batch_id, batchId, batch_code, batchCode, 
        // 前端直接发送的字段名（createTask/useTasks.ts apiTaskData）
        planStart, planEnd, field, assignee, 
        // 数据改造新增字段（落库保存）
        type_name, typeName, source_type, sourceType, dispatch_mode, dispatchMode, feedback_requirements, feedbackRequirements, requiredFeedback, rework_history, reworkHistory, deadline_extensions, deadlineExtensions, type_config, typeConfig, sop_content, sopContent, description, 
        // 班组与工具备注
        team_id, teamId, team_name, teamName, tools_remarks, toolsRemarks, 
        // 问题分派/巡查关联字段
        source_problem_id, sourceProblemId, source_inspection_id, sourceInspectionId, source_id, sourceId, source_code, sourceCode, } = req.body;
 ...
 * 根据任务编码获取任务
 */
router.get('/code/:taskCode', (req, res) => {
    try {
...
router.get('/stats', (req, res) => {
    try {
        ...
        const overdue = execCount(db, "SELECT COUNT(*) as count FROM farm_tasks WHERE plan_date < ? AND status NOT IN ('completed', 'cancelled', 'abandoned')", [today]);
        res.json({
            success: true,
            data: {
                total,
...
router.get('/overdue', (req, res) => {
    try {
        const db = getDatabase();
        const today = new Date().toISOString().split('T')[0];
        const items = queryToObjects(db, "SELECT * FROM farm_tasks WHERE plan_date < ? AND status NOT IN ('completed', 'cancelled', 'abandoned') ORDER BY plan_date DESC", [today]);
...
router.get('/pending', (req, res) => {
    try {
        const db = getDatabase();
        const items = queryToObjects(db, "SELECT * FROM farm_tasks WHERE status = 'pending' ORDER BY plan_date DESC, plan_time DESC", []);
        const itemsWithLabels = items.map((item) => ({
            ...item,
            statusLabel: getTaskStatusLabel(item.status || 'pending'),
        }));
...
router.get('/in-progress', (req, res) => {
    try {
        const db = getDatabase();
        const items = queryToObjects(db, "SELECT * FROM farm_tasks WHERE status = 'in_progress' ORDER BY plan_date DESC, plan_time DESC", []);
        const itemsWithLabels = items.map((item) => ({
            ...item,
            statusLabel: getTaskStatusLabel(item.status || 'pending'),
        }));
...
router.get('/waiting-acceptance', (req, res) => {
    try {
        const db = getDatabase();
        const items = queryToObjects(db, "SELECT * FROM farm_tasks WHERE status = 'waiting_acceptance' ORDER BY plan_date DESC, plan_time DESC", []);
        const itemsWithLabels = items.map((item) => ({
            ...item,
            statusLabel: getTaskStatusLabel(item.status || 'pending'),
        }));
...
router.get('/:id/records', (req, res) => {
    try {
        const { id } = req.params;
        const db = getDatabase();
        const items = queryToObjects(db, 'SELECT * FROM task_operation_records WHERE task_id = ? ORDER BY action_time DESC', [id]);
 ...
 * 记录任务操作
 */
function recordTaskOperation(db, taskId, taskCode, taskTitle, operatorId, operatorName, action, actionName, fromStatus, toStatus, progress, comment, reason) {
...
router.post('/:id/publish', (req, res) => {
    try {
        ...
        const now = new Date().toISOString();
        db.run(`UPDATE farm_tasks SET status = 'pending', update_time = ? WHERE id = ?`, [now, id]);
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'publish', '发布任务', fromStatus, 'pending');
        saveDatabase();
...
router.post('/:id/withdraw', (req, res) => {
    try {
        ...
        const fromStatus = task.status;
        const now = new Date().toISOString();
        db.run(`UPDATE farm_tasks SET status = 'draft', update_time = ? WHERE id = ?`, [now, id]);
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'withdraw', '撤回任务', fromStatus, 'draft');
        saveDatabase();
...
router.post('/:id/accept', (req, res) => {
    try {
        ...
        const fromStatus = task.status;
        const now = new Date().toISOString();
        db.run(`UPDATE farm_tasks SET status = 'accepted', accepted_at = ?, update_time = ? WHERE id = ?`, [now, now, id]);
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'accept', '接受任务', fromStatus, 'accepted');
        saveDatabase();
...
router.post('/:id/start', (req, res) => {
    try {
        ...
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'start', '开始执行', fromStatus, 'in_progress');
        saveDatabase();
...
router.post('/:id/progress', (req, res) => {
    try {
        ...
        db.run(`UPDATE farm_tasks SET progress = ?, feedback = ?, update_time = ? WHERE id = ?`, [progress || currentProgress, feedback ? JSON.stringify(feedback) : null, now, id]);
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'progress', '提交进度', task.status, task.status, progress, comment);
        saveDatabase();
 ...
 * 申请验收
 */
router.post('/:id/submit-acceptance', (req, res) => {
    try {
        const { id } = req.params;
        ...
        const fromStatus = task.status;
        const now = new Date().toISOString();
        db.run(`UPDATE farm_tasks SET status = 'waiting_acceptance', progress = 100, update_time = ? WHERE id = ?`, [now, id]);
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'submit', '申请验收', fromStatus, 'waiting_acceptance', 100, comment);
        saveDatabase();
...
router.post('/:id/complete', (req, res) => {
    try {
        ...
        const fromStatus = task.status;
        const now = new Date().toISOString();
        db.run(`UPDATE farm_tasks SET status = 'completed', completed_at = ?, progress = 100, update_time = ? WHERE id = ?`, [now, now, id]);
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'complete', '验收通过', fromStatus, 'completed', 100, comments);
        saveDatabase();
...
router.post('/:id/reject', (req, res) => {
    try {
        ...
        db.run(`UPDATE farm_tasks SET status = ?, rework_count = ?, update_time = ? WHERE id = ?`, [newStatus, reworkCount, now, id]);
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'reject', '验收驳回', fromStatus, newStatus, undefined, reason);
        saveDatabase();
...
router.post('/:id/cancel', (req, res) => {
    try {
        ...
        const fromStatus = task.status;
        const now = new Date().toISOString();
        db.run(`UPDATE farm_tasks SET status = 'cancelled', update_time = ? WHERE id = ?`, [now, id]);
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'cancel', '取消任务', fromStatus, 'cancelled', undefined, reason);
        saveDatabase();
...
router.post('/:id/abandon', (req, res) => {
    try {
        ...
        const fromStatus = task.status;
        const now = new Date().toISOString();
        db.run(`UPDATE farm_tasks SET status = 'abandoned', update_time = ? WHERE id = ?`, [now, id]);
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'abandon', '放弃任务', fromStatus, 'abandoned', undefined, reason);
        saveDatabase();
 ...
 * 超时继续
 */
router.post('/:id/overtime-continue', (req, res) => {
    try {
        const { id } = req.params;
        ...
        params.push(id);
        db.run(sql, params);
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'overtime_continue', '超时继续', task.status, task.status);
        saveDatabase();
        res.json({ success: true, data: { id } });
    }
    catch (error) {
 ...
 * 超时放弃
 */
router.post('/:id/overtime-abandon', (req, res) => {
    try {
        const { id } = req.params;
        const { reason, operator_id, operator_name } = req.body;
        const db = getDatabase();
        ...
        const now = new Date().toISOString();
        db.run(`UPDATE farm_tasks SET status = 'abandoned', update_time = ? WHERE id = ?`, [now, id]);
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'overtime_abandon', '超时放弃', fromStatus, 'abandoned', undefined, reason);
        saveDatabase();
        res.json({ success: true, data: { id, status: 'abandoned' } });
    }
    catch (error) {
...
router.post('/:id/reassign', (req, res) => {
    try {
        ...
        const now = new Date().toISOString();
        db.run(`UPDATE farm_tasks SET assignee_id = ?, assignee_name = ?, status = 'pending', update_time = ? WHERE id = ?`, [assigneeId, assigneeName, now, id]);
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'reassign', '重新派发', task.status, 'pending', undefined, reason);
        saveDatabase();
 ...
 * 延期任务
 */
router.post('/:id/extend-deadline', (req, res) => {
    try {
        const { id } = req.params;
        ...
        const now = new Date().toISOString();
        db.run(`UPDATE farm_tasks SET due_date = ?, update_time = ? WHERE id = ?`, [newDeadline, now, id]);
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'extend_deadline', '延期', task.status, task.status, undefined, reason);
        saveDatabase();
...
router.post('/:id/remind', (req, res) => {
    try {
        ...
        recordTaskOperation(db, id, task.task_code, task.task_title || task.title, operator_id || '', operator_name || '', 'remind', '催办', task.status, task.status, undefined, message);
        saveDatabase();
