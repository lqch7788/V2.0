...
import * as materialsDb from '../db/materials.js';
const router = Router();
// ==================== 物料管理 ====================
/**
 * 获取所有物料
...
router.get('/inbound', (req, res) => {
    try {
        ...
        const parsedRecords = records.map(record => ({
            ...record,
            materials: record.materials ? JSON.parse(record.materials).map((m) => ({
                ...m,
...
router.post('/inbound', (req, res) => {
    try {
        ...
        res.status(201).json({
            ...created,
            materials: created?.materials ? JSON.parse(created.materials).map((m) => ({
                ...m,
                code: m.code || m.materialCode || '',
                name: m.name || m.materialName || '',
            })) : []
...
router.put('/inbound/:id', (req, res) => {
    try {
        ...
        res.json({
            ...updated,
            materials: updated?.materials ? JSON.parse(updated.materials).map((m) => ({
                ...m,
                code: m.code || m.materialCode || '',
                name: m.name || m.materialName || '',
            })) : []
