...
router.get('/templates', (req, res) => {
    try {
        ...
        const { type, status, keyword, page = 1, limit = 50 } = req.query;
...
router.post('/templates', (req, res) => {
    try {
        const db = getDatabase();
        const { code, name, type, category, title_template, content, default_priority, status } = req.body;
        if (!name) {
        ...
        const templateCode = code || `TPL_${Date.now()}`;
        db.run(`
...
router.get('/', (req, res) => {
    try {
        const db = getDatabase();
        const { type, priority, status, keyword, page = 1, limit = 50 } = req.query;
...
router.post('/', (req, res) => {
    try {
        const db = getDatabase();
        const { id, code, title, type, category, priority, status, sender, date, deadline, readCount, recipients, content } = req.body;
        if (!id) {
