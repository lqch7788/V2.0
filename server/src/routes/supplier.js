...
const VALIDATION = {
  mobilePhone: (v) => !v || /^1[3|4|5|7|8][0-9]{9}$/.test(v),
  workPhone: (v) => !v || /^(\d{3,4}-)\d{7,8}$/.test(v) || /^\(\d{3,4}\)\d{7,8}$/.test(v),
  fax: (v) => !v || /^(\d{3,4}-)\d{7,8}$/.test(v) || /^\(\d{3,4}\)\d{7,8}$/.test(v) || /^1[3|4|5|7|8][0-9]{9}$/.test(v),
  bankCard: (v) => !v || /^([1-9])(\d{14}|\d{17,18})$/.test(v),
};

router.get('/', (req, res) => { supplier_name, status, page = 1, limit = 50 } = req.query;
    ...
    res.status(500).json({ success, error: '获取供应商失败' });
  }
});
...
router.get('/:id', (req, res) => { id } = req.params;
    ...
    if (!item || Object.keys(item).length === 0) {
      return res.status(404).json({ success, error: '供应商不存在' });
    }

    res.json({ success: true, data: items);
  } catch (error) {
    res.status(500).json({ success, error: '获取供应商详情失败' });
  }
});
...
router.post('/', (req, res) => {
    ...
    if (mobile_phone && !VALIDATION.mobilePhone(mobile_phone)) {
      return res.status(400).json({ success, error: '手机号格式不正确，应为1开头的11位数字' });
    }
    if (work_phone && !VALIDATION.workPhone(work_phone)) {
      return res.status(400).json({ success, error: '工作电话格式不正确，应为区号-号码格式' });
    }
    if (fax && !VALIDATION.fax(fax)) {
      return res.status(400).json({ success, error: '传真格式不正确' });
    }
    if (bank_card_number && !VALIDATION.bankCard(bank_card_number)) {
      return res.status(400).json({ success, error: '银行卡号格式不正确，应为15位或17-18位数字' });
    }
    ...
    res.status(500).json({ success, error: '创建供应商失败' });
  }
});
...
router.put('/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '更新供应商失败' });
  }
});
...
router.delete('/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '删除供应商失败' });
  }
});
