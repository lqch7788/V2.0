...
function mapFieldsToFrontend(item){
  ...
  const result= {
    ...
    batchStatus,
    statusClass=== 'published' ? 'normal' :
                 batchStatus === 'pending' ? 'pending' : 'draft',
    approveStatus=== 'published' || batchStatus === 'approved' ? '已审批' : '待审批',
...
router.get('/', (req, res) => {
    ...
    res.status(500).json({ success, error: '获取技术方案列表失败' });
  }
});
...
router.get('/:id', (req, res) => { id } = req.params;
    ...
    if (items && items.length > 0) {
      ...
      res.status(404).json({ success, error: '技术方案不存在' });
    }  } catch (error) {
    console.error('获取技术方案详情失败:', error);
    res.status(500).json({ success, error: '获取技术方案详情失败' });
  }
});
...
router.post('/', (req, res) => {
    ...
    res.status(500).json({ success, error: '创建技术方案失败' });
  }
});
...
router.put('/:id', (req, res) => { id } = req.params;
    ...
    res.json({ success, message: '技术方案更新成功', data);
  } catch (error) {
    console.error('更新技术方案失败:', error);
    res.status(500).json({ success, error: '更新技术方案失败' });
  }
});
...
router.delete('/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '删除技术方案失败' });
  }
});
...
router.post('/batch-delete', (req, res) => { ids } = req.body;
    ...
    if (!Array.isArray(ids) || ids.length === 0) {
      return res.status(400).json({ success, error: '请选择要删除的技术方案' });
    }
    ...
    res.status(500).json({ success, error: '批量删除技术方案失败' });
  }
});
...
router.get('/stats/summary', (req, res) => {
    ...
    const sql = `
      SELECT
        COUNT(*)(CASE WHEN batch_status = 'draft' THEN 1 ELSE 0 END)(CASE WHEN batch_status = 'pending' THEN 1 ELSE 0 END)(CASE WHEN batch_status = 'published' THEN 1 ELSE 0 END)(CASE WHEN batch_status = 'approved' THEN 1 ELSE 0 END)(CASE WHEN batch_status = 'rejected' THEN 1 ELSE 0 END)(CASE WHEN batch_status = 'cancelled' THEN 1 ELSE 0 END)`;
    ...
    res.status(500).json({ success, error: '获取技术方案统计失败' });
  }
});
