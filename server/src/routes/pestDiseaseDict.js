...
router.get('/next-code', (req, res) => { type } = req.query;
...
router.post('/', (req, res) => {
    ...
    if (!body.dict_name || !body.dict_type) {
      res.status(400).json({ success, error: '病虫害名称和类型为必填项' });
      return;
    }
...
router.delete('/:id', (req, res) => {
    ...
    stmt.bind([id]);
    stmt.step();
    const related = stmt.getAsObject(): number };
    stmt.free();

    if (related.count > 0) {
      return res.status(400).json({ success, error: '该病虫害存在关联药剂，无法删除' });
    }
...
router.post('/:pestId/relations', (req, res) => { pestId } = req.params;
    const { pesticideId } = req.body;

    if (!pesticideId) {
      return res.status(400).json({ success, error: 'pesticideId is required' });
    }
    ...
    try {
      ...
      if (e.message.includes('UNIQUE constraint')) {
        return res.status(400).json({ success, error: '该关联已存在' });
      }
      throw e;
    }
  } catch (error) {
...
router.delete('/:pestId/relations/:pesticideId', (req, res) => { pestId, pesticideId } = req.params;
    const db = getDatabase();
