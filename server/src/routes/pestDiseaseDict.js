...
router.get('/next-code', (req, res) => {
    ...
    if (!type || !['pest', 'disease'].includes(type)) {
        return res.status(400).json({ error: 'type must be pest or disease' });
    }
    const prefix = type === 'pest' ? 'PD-P-' : 'PD-D-';
    const db = getDatabase();
    const stmt = db.prepare(`SELECT dict_code FROM pest_disease_dict WHERE dict_type = ? AND dict_code LIKE ? ORDER BY dict_code DESC LIMIT 1`);
...
function generateDictCode(db, dictType) {
    const prefix = dictType === 'pest' ? 'PD-P-' : 'PD-D-';
    const allCodes = queryToObjects(db, `SELECT dict_code FROM pest_disease_dict WHERE dict_type = ?`, [dictType]);
    let maxSeq = 0;
...
router.get('/', (req, res) => {
    try {
        const db = getDatabase();
        const { dictType, dict_name, keyword, page = '1', limit = '20' } = req.query;
        ...
        const items = queryToObjects(db, `SELECT * FROM pest_disease_dict ${whereClause} ORDER BY create_time DESC LIMIT ? OFFSET ?`, [...params, limitNum, offset]);
...
router.post('/', (req, res) => {
    ...
    ) VALUES (?,?,?,?,?,?,?,?)`, [id, code, body.dict_name, body.dict_type, body.target_crops || null,
...
router.put('/:id', (req, res) => {
    try {
        ...
        const { id } = req.params;
        const body = req.body;
        const existing = queryToObjects(db, `SELECT * FROM pest_disease_dict WHERE id = ?`, [id]);
        if (existing.length === 0) {
            res.status(404).json({ success: false, error: '记录不存在' });
            return;
        }
        db.run(`UPDATE pest_disease_dict SET dict_name=?, dict_type=?, target_crops=?,
      description=?, status=? WHERE id=?`, [body.dict_name ?? existing[0].dict_name, body.dict_type ?? existing[0].dict_type,
...
router.delete('/:id', (req, res) => {
    try {
        ...
        const { id } = req.params;
        // 检查是否有关联药剂
        const stmt = db.prepare('SELECT COUNT(*) as count FROM pesticide_pest_relation WHERE pest_id = ?');
        stmt.bind([id]);
        stmt.step();
        const related = stmt.getAsObject();
        stmt.free();
        if (related.count > 0) {
...
router.get('/by-crop/:cropName', (req, res) => {
    try {
        const db = getDatabase();
        const { cropName } = req.params;
        const items = queryToObjects(db, `SELECT * FROM pest_disease_dict WHERE target_crops LIKE ? ORDER BY dict_type, dict_name`, [`%${cropName}%`]);
...
router.post('/:pestId/relations', (req, res) => {
    try {
...
router.delete('/:pestId/relations/:pesticideId', (req, res) => {
    try {
