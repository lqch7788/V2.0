...
router.get('/', (req, res) => { status, applicant, department, page = 1, limit = 50 } = req.query;
    ...
    res.status(500).json({ success, error: '获取退料列表失败' });
  }
});
...
router.get('/:id', (req, res) => { id } = req.params;
    ...
    if (!item || Object.keys(item).length === 0) {
      return res.status(404).json({ success, error: '退料记录不存在' });
    }
    item.materials = item.materials ? JSON.parse(item.materials) : [];
    res.json({ success: true, data: result);
  } catch (error) {
    res.status(500).json({ success, error: '获取退料详情失败' });
  }
});

// POST /api/material-returns - 创建退料记录
router.post('/', (req, res) => { id, code, date, type, applicant, department, warehouseLocation, status, statusClass,
    ...
    res.status(500).json({ success, error: '创建退料失败' });
  }
});
...
router.put('/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '更新退料失败' });
  }
});
...
router.delete('/:id', (req, res) => { id } = req.params;
    ...
    res.status(500).json({ success, error: '删除退料失败' });
  }
});
