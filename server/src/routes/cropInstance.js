...
router.post('/', (req, res) => {
    try {
        const { id, instance_code, order_id, order_code, crop_category, crop_name, crop_variety, category_code, type_code, sub_code, source_origin, source_description, initial_quantity, current_quantity, planted_quantity, harvested_quantity, status, seed_entry_date, seedling_start_date, planting_date, harvest_date, source_instance_id, create_by } = req.body;
...
// 获取实例的完整溯源链
router.get('/:id/trace-chain', (req, res) => {
    try {
        const { id } = req.params;
        const db = getDatabase();
        // 获取作物实例基本信息
