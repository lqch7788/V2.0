...
router.get('/', (req, res) => {
    ...
    res.status(500).json({ success, error: '获取设备分配列表失败' });
  }
});
...
router.post('/', (req, res) => {
    ...
    if (!device_name) return res.status(400).json({ success, error: '设备名称不能为空' });
    ...
    res.status(500).json({ success, error: '创建设备分配失败' });
  }
});
...
router.put('/:oid', (req, res) => {
    ...
    if (result.length === 0 || result[0].values.length === 0) {
      return res.status(404).json({ success, error: '分配记录不存在' });
    }
    const columns = result[0].columns;
    const record= {};
    columns.forEach((col, i) => { record[col] = result[0].values[0][i]; });
    ...
    res.status(500).json({ success, error: '更新设备分配失败' });
  }
});
...
router.delete('/:oid', (req, res) => {
    ...
    res.json({ success, message: '分配记录已删除' });
  } catch (error) {
    console.error('删除设备分配失败:', error);
    res.status(500).json({ success, error: '删除设备分配失败' });
  }
});
