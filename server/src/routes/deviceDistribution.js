...
router.post('/', (req, res) => {
    try {
        const db = getDatabase();
        const { device_name, device_code, site_name, area_name, device_type, motor_name, sort_order, allow_runtime, rest_time, initial_status, circuit, slave_devices, start_time, show_curve, specs, remarks } = req.body;
        if (!device_name)
...
router.put('/:oid', (req, res) => {
    try {
        const db = getDatabase();
        const { device_name, device_code, site_name, area_name, device_type, motor_name, sort_order, allow_runtime, rest_time, initial_status, circuit, slave_devices, start_time, show_curve, specs, remarks, status } = req.body;
        const now = new Date().toISOString();
        db.run(`
