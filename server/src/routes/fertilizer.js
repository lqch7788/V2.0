...
import { iotIngestSchema } from '../validation/iotIngest.js';
const router = Router();
/** 生成施肥编号 SF+年月日-4位流水号 */
function generateFertilizerCode(db) {
    ...
    const allCodes = queryToObjects(db, `SELECT fertilizer_code FROM fertilizer_records`);
    let maxSeq = 0;
...
router.post('/batch-delete', (req, res) => {
    try {
        ...
        const iotRows = queryToObjects(db, `SELECT id FROM fertilizer_records WHERE id IN (${placeholders}) AND data_source = 'auto_iot'`, ids);
...
router.get('/', (req, res) => {
    try {
        const db = getDatabase();
        const { fertilizer_name, fertilizer_type, crop_name, greenhouse_name, data_source, start_date, end_date, operator_name, planting_code, page = '1', limit = '20' } = req.query;
        const pageNum = Math.max(1, parseInt(page, 10) || 1);
        const limitNum = Math.min(100, Math.max(1, parseInt(limit, 10) || 20));
        const conditions = [];
        const params = [];
        if (fertilizer_name) {
            conditions.push("fertilizer_name LIKE '%' || ? || '%'");
            params.push(fertilizer_name);
        }
        if (fertilizer_type) {
            conditions.push('fertilizer_type = ?');
            params.push(fertilizer_type);
        }
        if (crop_name) {
        ...
        if (greenhouse_name) {
            conditions.push("greenhouse_name LIKE '%' || ? || '%'");
        ...
        if (start_date) {
            conditions.push('fertilize_time >= ?');
        ...
        if (planting_code) {
            conditions.push("planting_code LIKE '%' || ? || '%'");
            params.push(planting_code);
        }
        const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
        const total = execCount(db, `SELECT * FROM fertilizer_records ${whereClause}`, params);
        const offset = (pageNum - 1) * limitNum;
        const items = queryToObjects(db, `SELECT * FROM fertilizer_records ${whereClause} ORDER BY fertilize_time DESC LIMIT ? OFFSET ?`, [...params, limitNum, offset]);
...
router.post('/', (req, res) => {
    ...
    ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [id, code, body.farm_task_id || null, body.production_plan_id || null, body.production_plan_code || null,
...
router.get('/stats', (req, res) => {
    try {
        ...
        if (greenhouse_name) {
            conditions.push('greenhouse_name = ?');
            params.push(greenhouse_name);
        }
        const whereClause = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
        let groupField;
        switch (group_by) {
            case 'crop':
                groupField = 'crop_name';
                break;
            case 'fertilizer_type':
                groupField = 'fertilizer_type';
                break;
            case 'greenhouse':
                groupField = 'greenhouse_name';
                break;
            default:
                groupField = "strftime('%Y-%m', fertilize_time)";
                break;
        }
        const items = queryToObjects(db, `SELECT ${groupField} as label, COUNT(*) as record_count,
...
router.post('/iot-ingest', iotAuth, (req, res) => {
    try {
        ...
        for (const record of records) {
            const dups = queryToObjects(db, `SELECT id FROM fertilizer_records WHERE iot_record_id = ? AND iot_device_id = ?`, [record.iot_record_id, device_id]);
            if (dups.length > 0) {
                skipped++;
                continue;
            }
      ...
      ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [id, code, record.greenhouse_name || '', record.area_name || null, record.crop_name || '',
...
router.put('/:id', (req, res) => {
    try {
        ...
        const { id } = req.params;
        const body = req.body;
        const existing = queryToObjects(db, `SELECT * FROM fertilizer_records WHERE id = ?`, [id]);
        if (existing.length === 0) {
            res.status(404).json({ success: false, error: '记录不存在' });
            return;
        }
        if (existing[0].data_source === 'auto_iot') {
            res.status(403).json({ success: false, error: 'IoT自动记录不可编辑' });
            return;
        }
        const now = new Date().toISOString();
      ...
      planting_id=?, planting_code=?, update_time=? WHERE id=?`, [body.fertilizer_name ?? existing[0].fertilizer_name, body.fertilizer_type ?? existing[0].fertilizer_type,
...
router.delete('/:id', (req, res) => {
    try {
        ...
        if (existing[0].data_source === 'auto_iot') {
            res.status(403).json({ success: false, error: 'IoT自动记录不可删除' });
            return;
        }
