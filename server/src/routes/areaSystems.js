...
router.get('/', (req, res) => {
    try {
        ...
        const params = [];
        if (partition_oid) {
            conditions.push('asm.partition_oid = ?');
            params.push(partition_oid);
        }
        if (system_oid) {
            conditions.push('asm.system_oid = ?');
            params.push(system_oid);
        }
...
router.post('/', (req, res) => {
    try {
        ...
        // 检查是否已存在
        const existing = db.exec('SELECT oid FROM area_system_mappings WHERE partition_oid = ? AND system_oid = ?', [partition_oid, system_oid]);
