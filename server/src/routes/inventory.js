 ...
 * 获取所有库存记录
 */
router.get('/', (req, res) => { crop_name, stock_type, status, page = 1, limit = 50 } = req.query;
 ...
 * 按作物名称聚合查询库存（多形态搜索）
 */
router.get('/aggregate/by-crop', (req, res) => { crop_name } = req.query;
    const db = getDatabase();
...
router.get('/batch', (req, res) => { ids } = req.query;
    ...
    if (idArray.length === 0) {
      return res.json({ success: true, data: camelItems);
    }
    ...
    res.status(500).json({ success, error: '批量获取库存记录失败' });
  }
});
...
router.put('/batch', (req, res) => { ids, updates } = req.body;
    ...
    res.status(500).json({ success, error: '批量更新库存记录失败' });
  }
});
...
router.delete('/batch', (req, res) => { ids } = req.query;
    ...
    res.status(500).json({ success, error: '批量删除库存记录失败' });
  }
});
