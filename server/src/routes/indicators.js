...
router.get('/', (req, res) => {
    ...
    res.status(500).json({ success, error: '获取指标列表失败' });
  }
});
...
router.get('/:id', (req, res) => { id } = req.params;
    ...
    if (!item || Object.keys(item).length === 0) {
      return res.status(404).json({ success, error: '指标不存在' });
    }

    res.json({ success: true, data: items);
  } catch (error) {
    console.error('获取指标详情失败:', error);
    res.status(500).json({ success, error: '获取指标详情失败' });
  }
});
...
router.post('/', (req, res) => {
    ...
    if (!id) {
      return res.status(400).json({ success, error: '指标ID不能为空' });
    }
    ...
    res.status(201).json({ success, message: '指标创建成功', data);
  } catch (error) {
    console.error('创建指标失败:', error);
    res.status(500).json({ success, error: '创建指标失败' });
  }
});
...
router.put('/:id', (req, res) => { id } = req.params;
    ...
    res.json({ success, message: '指标更新成功', data);
  } catch (error) {
    console.error('更新指标失败:', error);
    res.status(500).json({ success, error: '更新指标失败' });
  }
});
...
router.delete('/:id', (req, res) => { id } = req.params;
    ...
    res.json({ success, message: '指标删除成功' });
  } catch (error) {
    console.error('删除指标失败:', error);
    res.status(500).json({ success, error: '删除指标失败' });
  }
});
...
router.delete('/batch', (req, res) => { ids } = req.body;
    ...
    if (!Array.isArray(ids) || ids.length === 0) {
      return res.status(400).json({ success, error: '请提供要删除的指标ID列表' });
    }
    ...
    res.json({ success, message: `成功删除 ${ids.length} 个指标` });
  } catch (error) {
    console.error('批量删除指标失败:', error);
    res.status(500).json({ success, error: '批量删除指标失败' });
  }
});
...
router.post('/reset', (req, res) => {
    ...
    saveDatabase();
    res.json({ success, message: '指标数据已重置' });
  } catch (error) {
    console.error('重置指标数据失败:', error);
    res.status(500).json({ success, error: '重置指标数据失败' });
  }
});
 ...
 */
router.post('/import', (req, res) => { indicators } = req.body;

    if (!Array.isArray(indicators) || indicators.length === 0) {
      return res.status(400).json({ success, error: '请提供有效的指标数据' });
    }
    ...
    saveDatabase();
    res.json({ success, message: `成功导入 ${indicators.length} 条指标数据` });
  } catch (error) {
    console.error('导入指标数据失败:', error);
    res.status(500).json({ success, error: '导入指标数据失败' });
  }
});
