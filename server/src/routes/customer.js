...
router.get('/', (req, res) => {
    ...
    res.status(500).json({ success, error: '获取客户列表失败' });
  }
});
...
router.get('/:id', (req, res) => { id } = req.params;
    ...
    if (!items || items.length === 0) {
      return res.status(404).json({ success, error: '客户不存在' });
    }
    res.json({ success: true, data: items);
  } catch (error) {
    console.error('获取客户详情失败:', error);
    res.status(500).json({ success, error: '获取客户详情失败' });
  }
});
...
router.post('/', (req, res) => {
    ...
    if (!finalCustomerCode) {
      const existingCodes = queryToObjects<{ customerCode: string }>(
        db,
        'SELECT customer_code AS customerCode FROM customers',
        []
    ...
    const newCustomers = queryToObjects<Record<string, unknown>>(
      db,
      'SELECT * FROM customers WHERE id = ?',
      [id]
    );
    ...
    res.status(500).json({ success, error: '创建客户失败' });
  }
});
...
router.put('/:id', (req, res) => { id } = req.params;
    ...
    // 检查客户是否存在
    const existing = queryToObjects<Record<string, unknown>>(
      db,
      'SELECT * FROM customers WHERE id = ?',
      [id]
    );
    ...
    const updated = queryToObjects<Record<string, unknown>>(
      db,
      'SELECT * FROM customers WHERE id = ?',
      [id]
    );
    ...
    res.status(500).json({ success, error: '更新客户失败' });
  }
});
...
router.delete('/:id', (req, res) => { id } = req.params;
    ...
    res.json({ success, message: '客户删除成功' });
  } catch (error) {
    console.error('删除客户失败:', error);
    res.status(500).json({ success, error: '删除客户失败' });
  }
});
