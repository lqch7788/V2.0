...
function generatePesticideCode(db, controlType){
  ...
  const codes= [];
  while (stmt.step()) {
    const obj = stmt.getAsObject(): string };
...
router.post('/', (req, res) => {
    ...
    if (!body.pesticide_name || !body.control_type) {
      res.status(400).json({ success, error: '药剂名称和防治类型为必填项' });
      return;
    }
...
router.put('/:id', (req, res) => {
    ...
    if (existing.length === 0) { res.status(404).json({ success, error: '药剂不存在' }); return; }
...
router.put('/specs/:specId', (req, res) => {
    ...
    if (existing.length === 0) { res.status(404).json({ success, error: '规格不存在' }); return; }
...
router.put('/:id/relations', (req, res) => { id } = req.params;
    const { pestIds } = req.body;

    if (!Array.isArray(pestIds)) {
      return res.status(400).json({ success, error: 'pestIds must be an array' });
    }
...
router.delete('/:id/relations/:pestId', (req, res) => { id, pestId } = req.params;
    const db = getDatabase();
