...
router.get('/', (req, res) => {
    ...
    res.status(500).json({ success, error: '获取设备系统列表失败' });
  }
});
...
router.post('/', (req, res) => {
    ...
    if (!system_code || !system_name) {
      return res.status(400).json({ success, error: '系统编码和名称不能为空' });
    }

    // 检查编码唯一性
    const existing = db.exec('SELECT oid FROM device_systems WHERE system_code = ?', [system_code]);
    if (existing.length > 0 && existing[0].values.length > 0) {
      return res.status(409).json({ success, error: '该系统编码已存在' });
    }
    ...
    res.status(500).json({ success, error: '创建设备系统失败' });
  }
});
...
router.put('/:oid', (req, res) => {
    ...
    if (result.length === 0 || result[0].values.length === 0) {
      return res.status(404).json({ success, error: '系统不存在' });
    }
    const columns = result[0].columns;
    const record= {};
    columns.forEach((col, i) => { record[col] = result[0].values[0][i]; });
    ...
    res.status(500).json({ success, error: '更新设备系统失败' });
  }
});
...
router.delete('/:oid', (req, res) => {
    ...
    res.json({ success, message: '系统已删除' });
  } catch (error) {
    console.error('删除设备系统失败:', error);
    res.status(500).json({ success, error: '删除设备系统失败' });
  }
});
