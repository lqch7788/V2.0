...
export function deductLeaveQuota(db, leaveRecord) {
    try {
        ...
        db.run('UPDATE leave_quotas SET used_days = ?, frozen_days = ?, remaining_days = ?, update_time = ? WHERE worker_id = ? AND year = ? AND leave_category = ?', [newUsedDays, newFrozenDays, newRemainingDays, new Date().toISOString(), worker_id, year, leaveCategory]);
...
export function deductOvertimeQuota(db, overtimeRecord) {
    try {
        ...
        if (!quota) {
            ...
            db.run('INSERT INTO leave_quotas (id, worker_id, worker_name, year, leave_category, total_days, used_days, frozen_days, remaining_days, create_time, update_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [newId, worker_id, overtimeRecord.worker_name || '', year, leaveCategory, 0, Number(hours), 0, 0, now, now]);
            console.log(`[Quota] Overtime quota created: worker=${worker_id}, hours=${hours}`);
            return true;
        }
        const currentUsed = Number(quota.used_days) || 0;
        const newUsedHours = currentUsed + Number(hours);
        db.run('UPDATE leave_quotas SET used_days = ?, update_time = ? WHERE worker_id = ? AND year = ? AND leave_category = ?', [newUsedHours, new Date().toISOString(), worker_id, year, leaveCategory]);
...
export function initEmployeeQuotas(db, employeeId, employeeName, year) {
    try {
        ...
        for (const { category, totalDays } of defaultLeaveCategories) {
            ...
            if (!exists) {
                const newId = generateId('LQ');
                db.run('INSERT INTO leave_quotas (id, worker_id, worker_name, year, leave_category, total_days, used_days, frozen_days, remaining_days, create_time, update_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [newId, employeeId, employeeName, targetYear, category, totalDays, 0, 0, totalDays, now, now]);
            }
        }
        ...
        if (!overtimeExists) {
            const newId = generateId('LQ');
            db.run('INSERT INTO leave_quotas (id, worker_id, worker_name, year, leave_category, total_days, used_days, frozen_days, remaining_days, create_time, update_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [newId, employeeId, employeeName, targetYear, 'overtime', 0, 0, 0, 0, now, now]);
        }
...
export function freezeLeaveQuota(db, leaveRecord) {
    try {
        ...
        db.run('UPDATE leave_quotas SET frozen_days = ?, remaining_days = ?, update_time = ? WHERE worker_id = ? AND year = ? AND leave_category = ?', [newFrozenDays, newRemainingDays, new Date().toISOString(), worker_id, year, leaveCategory]);
