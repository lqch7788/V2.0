...
export class PurchasePlanService {
  ...
  getStatusOptions() {
    const STATUS_VALUES = ['draft', 'pending', 'approved', 'purchasing', 'completed', 'cancelled'];
    const STATUS_TEXT = {
      draft: '草稿', pending: '待审批', approved: '已通过', in_progress: '执行中',
      purchasing: '采购中', completed: '已完成', cancelled: '已作废', rejected: '已拒绝',
    };
    return STATUS_VALUES.map(v => ({ value: v, label: STATUS_TEXT[v] || v }));
  }
  getPriorityOptions() {
    ...
    return PRIORITY_VALUES.map(v => ({ value: v, label: PRIORITY_TEXT[v] || v }));
  }
  getPurchaseTypeOptions() {
    const PURCHASE_TYPE_VALUES = ['production', 'urgent', 'routine', 'safety', 'material', 'equipment', 'other'];
    const PURCHASE_TYPE_TEXT = {
      production: '生产物资采购', urgent: '紧急采购', routine: '常规采购',
      safety: '劳保用品', material: '通用物资', equipment: '设备采购', other: '其他',
    };
    return PURCHASE_TYPE_VALUES.map(v => ({ value: v, label: PURCHASE_TYPE_TEXT[v] || v }));
  }
