    ...
    try {
      ...
      for (const item of data) {
        ...
        stmt.bind([
          ...
          item.worker || item.workerName || item.worker_name || '',
          item.taskType || item.task_type || item.type || '',
          item.date || item.workDate || item.work_date || '',
          item.workloadHours || item.workHours || item.work_hours || 0,
          item.hourlyRate || item.hourly_rate || 0,
          item.totalAmount || item.total_amount || 0,
          item.greenhouseId || item.greenhouse_id || '',
          item.greenhouse || item.greenhouseName || item.greenhouse_name || '',
          item.areaName || item.area_name || '',
          item.tasks || item.taskDescription || item.task_description || '',
          item.batchId || item.batch_id || '',
          item.batchCode || item.batch_code || '',
          item.status || 'pending',
