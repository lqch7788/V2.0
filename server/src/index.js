...
import { fixMissingSchema, deduplicateDictionaries } from './db/fixMissingSchema.js';
...
async function start() {
    try {
        ...
        if (process.env.SEED_DB === '1') {
            ...
            const { exportBasicData } = await import('./db/seedBasicData.js');
            exportBasicData();
            console.log('正在导入种子数据...');
            const { exportDatabase } = await import('./db/seedData.js');
            exportDatabase();
