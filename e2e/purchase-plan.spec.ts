import { test, expect } from '@playwright/test'
import { captureUISnapshot, assertParity } from './helpers'

/**
 * 采购计划 V1.1 vs V2.0 parity E2E
 * URL: /purchase-plan
 */
test.describe('采购计划 parity (V1.1 vs V2.0)', () => {
  test('列表页 UI 元素 1:1', async ({ page }, testInfo) => {
    await page.goto('/purchase-plan')
    const snapshot = await captureUISnapshot(page)
    expect(snapshot.title).toBeTruthy()
    // 至少存在 新建/搜索/重置 等基础按钮
    expect(snapshot.buttons.some((b) => b.includes('新建') || b.includes('添加'))).toBe(true)
    testInfo.annotations.push({ type: 'module', description: 'purchase-plan-list' })
  })

  test('V1.1 vs V2.0 同一 URL 行为一致', async ({ browser }) => {
    const ctxV11 = await browser.newContext({ baseURL: 'http://localhost:5188' })
    const ctxV20 = await browser.newContext({ baseURL: 'http://localhost:5000' })
    const p11 = await ctxV11.newPage()
    const p20 = await ctxV20.newPage()
    try {
      const s11 = await captureUISnapshot(await p11.goto('/purchase-plan').then(() => p11))
      const s20 = await captureUISnapshot(await p20.goto('/purchase-plan').then(() => p20))
      assertParity(s11, s20, 'purchase-plan')
    } finally {
      await ctxV11.close()
      await ctxV20.close()
    }
  })
})
