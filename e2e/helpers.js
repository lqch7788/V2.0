 ...
 */
export async function captureUISnapshot(page) {
  await page.waitForLoadState('networkidle')
  const buttons = await page.locator('button, .el-button:visible').allTextContents()
 ...
 * 创建双上下文（V1.1 + V2.0）
 */
export async function createV11V20Contexts(browser) {
 ...
 * 用于验证状态机转换 1:1
 */
export async function captureStatusValues(page) {
  await page.waitForLoadState('networkidle')
  // 尝试从状态徽章/标签捕获常见状态
  ...
  const selectValues = await page.locator('select, .el-select').evaluateAll((els) =>
    els.map((el) => el.value || el.textContent || '').filter(Boolean)
  )
  return {
